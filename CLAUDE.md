...
⚠️ **The three `*-authenticated-authorization` services share their body and keep their ports. Decided and shipped 2026-08-07.** The user asked whether they could collapse into one the way logout has; the answer, argued in full in `docs/decisions/authorization-service-consolidation.md`, was to remove the duplication without removing the deployables. Since `marketplace-common@4.4.0` the session lookup, the account re-read and the token rotation are `resolveAuthorizationSession`, `findAccountForSession` and `refreshSessionTokens`, and each service supplies only its own `TIER.*` constant, its own model and its own projection. Two things follow. **Merging the three into one process is a decision the user has already taken, against** — dispatching on a tier read out of the session is the pattern *Terminology* rejects, and one `process.exit(1)` for three tiers is an availability cost paid by customers; do not re-open it as a refactor. And **`marketplace-common` now has a Koa/GraphQL-shaped surface**, consumed by three of the nine services but deployed to all nine, so an edit there is wider than it looks — including for `vitest.mutation.config.mts`, which must inline both `@thedoctorweb_agency/marketplace-common` and `@axiumine/koa-utils` or a `vi.mock` of a koa-utils subpath silently stops intercepting.
...
one of the thirteen eslint/prettier repos — `tsc` runs as the first half of its own `test:cov`, so a type error
fails the coverage gate before a single test executes), and it had neither a mutation gate nor a
`stryker.config.mjs` until 2026-08-07.

⚠️ **Having no repo of its own is what left it ungated, and the fix lives in `.githooks/` up here.** Every
other package carries hooks beside its own `package.json`; services-status carried a `qodana.yaml`, a
`stryker.config.mjs` and a 100% coverage threshold with **nothing that ran any of them** — the appearance of a
gate rather than a gate. The parent's `pre-commit` now runs `yarn test:cov` and Qodana for it, scoped to
...
- ⚠️ **A per-machine environment file that is *wrong* fails where nothing is looking, and on 2026-08-07 two of them were.** The audit that finally ran the user tier's suites — parse each service's `REQUIRED_ENV_VARS` out of `src/index.mts`, then check that repo's local config for absent-or-empty, then fingerprint every shared key across all nine to find disagreement — found that both `*-user-authenticated-*` files were copies of an unrelated older project. Five `MONGO_TEST_*` keys were empty, which at least aborted loudly in `globalSetup`; three more were populated and wrong, which did not. `MONGODB_URI` named `testRnApollo` with no `authSource`, so the user tier was not reading `dbMarketplaceDev` at all. `INTROSPECTION_CODE` did not match the other seven, breaking the service-to-service bypass in both directions. And **`KEYGRIP_KEY_1`/`_2` did not match `marketplace-dev-public-authorization`'s** — that service signs the customer refresh cookie at `loginUser` and the user-tier authorization service verifies it, so every customer refresh returned 401 while both repos' suites stayed green, because each one signs and verifies with itself. **No test on this platform spans two services, so every cross-repo value agreement is unenforced by construction** — check it with a fingerprint sweep, not by reading files. `checkRequiredEnv` is `if (!env[envVar])`, so an empty value fails exactly like a missing one; that is the only class of these the code catches.
- ⚠️ **Quote any value containing whitespace in one of those files.** dotenv terminates a bare value at the first space or `#`, hands back the truncated prefix and reports nothing — which is how an 89-character Keygrip key became a 76-character slice of itself, still signing, still verifying, just never the same as anyone else's. Use single quotes: dotenv expands `\n` and `\r` escapes inside double quotes. Inspect these files only through a script that prints key names and salted-hash fingerprints — `sha256(key + ' ' + value)`, first six hex — never values; a fingerprint proves two repos agree without putting the secret in a terminal, and everything printed here is written in plaintext to `~/.claude/projects/<slug>/*.jsonl`.
- ⚠️ **A vitest project with no matching files passes, and on 2026-08-07 that cost two production bugs.** `marketplace-dev-user-authenticated-resource` declared an `integration` project and a `globalSetup.mts` and carried **zero `*.itest.mts`**, so it collected nothing and reported success — indistinguishable in the output from a suite that ran, while the repo showed 100% coverage and a 100 mutation score. Five files and 60 tests were written that day, and **their first run found that every address delete on the customer tier answered 500**, for two independent reasons the unit suite could not have caught because it mocks `User.updateOne`: **Mongoose 9 refuses an array update** unless `{ updatePipeline: true }` is passed (`Cannot pass an array to query updates unless the 'updatePipeline' option is set.`, thrown before the driver is reached), and **Mongoose casts a query filter against the schema but casts nothing inside a pipeline** — a pipeline is an opaque aggregation expression to it, `GraphQLID` resolves to a *string*, and `{ $ne: ['$$this._id', '68b1…'] }` compares an ObjectId to a string, is never equal, and answers `matchedCount: 1, modifiedCount: 0`. `funUserAddressDel` is the only pipeline update on the platform, so nothing else was exposed; coerce with `new Types.ObjectId(…)` before any id enters one. `marketplace-dev-user-authenticated-authorization` carries 1 integration file where the other eight services carry 3–5. **Count the files, not the checkmarks.**
