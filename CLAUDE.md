...
- ⚠️ **A vitest project with no matching files passes.** `marketplace-dev-user-authenticated-resource` declares an `integration` project and a `globalSetup.mts` and has **zero `*.itest.mts`**, so it collects nothing and reports success — indistinguishable in the output from a suite that ran. Everything that service exists for (`me`, personal data, address CRUD, the `defaultAddress` pointer) is proven against mocks and never against the real `$jsonSchema` or the `$expr` that rejects a dangling pointer, which are precisely what a mock cannot have an opinion about. The other eight services carry 3–4 integration files each; `marketplace-dev-user-authenticated-authorization` carries 1. Count the files, not the checkmarks.
