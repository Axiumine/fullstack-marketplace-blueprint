...
⚠️ **Eleven of the twelve repos are empty, and all twelve are remote-less. Verified 2026-08-04.** `git rev-list --count HEAD` returns nothing in any of them except `marketplace-shopowner`, which was created here rather than copied over and carries one commit on `feat/shopowner-portal`; `git remote get-url origin` returns nothing anywhere. This workspace is a rebranded *copy* of `/media/nvme/websites/pizzati`, made by renaming directories and sweeping identifiers; the copy carried the working trees but not the histories or the remotes.
