...
- The parent dir is its **own git repo**, tracking only workspace files (`CLAUDE.md`, `.claude/`, `.agents/`). Its `.gitignore` excludes `/BEs/`, `/marketplace-admin/` and `/marketplace-shopowner/`, so the sub-repos nest without conflict.

## Repo layout

Polyrepo, **not** a monorepo. Eleven independent **git** repos under this parent dir (which is itself a twelfth, tracking workspace files only).

⚠️ **All twelve repos are remote-less, and their history starts here. Verified 2026-08-04.** `git remote get-url origin` returns nothing anywhere. This workspace is a rebranded *copy* of `/media/nvme/websites/pizzati`, made by renaming directories and sweeping identifiers; the copy carried the working trees but neither the histories nor the remotes.

**Every repo now has a `main`, and it is the current state.** That was not true for most of the life of this workspace — the repos were committed for the first time during the 2026-08-04 rename, and even then only onto `refactor/italian-to-english`, so `main` existed nowhere until it was created at each branch tip on the user's explicit instruction. Ten repos carry a `chore: baseline before Italian to English rename` root commit and the rename on top; `marketplace-dev-authenticated-logout` and `marketplace-shopowner` carry one commit each. The rename branch was deleted everywhere it had been merged, which is every repo that had one.

Three consequences, all of which bite:

- **Rollback exists, but it is shallow.** There is exactly one pre-rename baseline per repo and nothing before it, so `git diff`, `git stash` and `git reset` work while `git log` answers almost nothing about *why* anything is the way it is. The 2026-08-04 catalogue deletion was carried out before any of these commits existed, with no baseline at all, on the user's explicit instruction after the risk was stated — it is in no repo's history and cannot be reverted.
- **Ten repos have uncommitted work sitting on `fix/githooks`**, the hook hardening that has not been committed anywhere but the parent. `git status` is meaningful again, but a clean tree is not the norm here yet.
- **The `never commit on main` rule still applies**, and now for the ordinary reason rather than a special one: `main` exists in all twelve and is checked out in the two clean repos, so a bare `git commit` lands on it. Branch first, exactly as everywhere else.

The original workspace **is** on GitHub — private repos under the **`Pizzati-Org`** org, one per directory. Those remotes belong to the other tree. Nothing here points at them, and nothing here should be pushed to them: the catalogue deletion and the rename would land on the live project. Deciding where these twelve repos get published, and under which org, is the user's call and has not been made.
...
## Frontends — marketplace-admin and marketplace-shopowner (React, current)
...
`marketplace-shopowner` is the second app and a **mirror of the first**: same stack, same conventions, same
hooks, dev port **3044**, pointed at the four services *without* `admin` in the name (4028 / 4029 / 4026 / 4030).
It is deliberately thinner, because the tier behind it is: `marketplace-dev-authenticated-resource` exposes
**one query and three mutations** in total (`shopOwnerCompanies`, `companyAdd`, `companyUpdate`, `companyDel`),
so the operator app's profile, password-change, personal-data, statistics and paginated-table screens have no
counterpart and were pruned rather than stubbed. Read its own `CLAUDE.md`, `README.md` and `COVERAGE.md` first.
...
⚠️ **`marketplace-shopowner` carries no `test/` directory**, per the user's standing "skip all tests"
instruction — the harness is in place, the tests are not, so its coverage gate reports 0% and a commit there
needs `--no-verify`. Do not lower a threshold or remove a gate to work around it. The customer frontend is a
third app that has not been started; copy it from one of these two, not from a shell.
...
- **`engines.node` is `^24.18.0` in every repo that has a `package.json`** — eleven of them; the parent workspace has none. Unified, and worth keeping that way. It used to be spelled four different ways (`^24.14.0`, `^24.14`, `24.14`, `24.14.0`) with two repos omitting the block entirely, and the two spellings without a caret are *not* "24.14 or newer": semver reads `24.14` as `24.14.x` and bare `24.14.0` as that one release. Three services therefore rejected Node 24.18.0 outright while the others installed fine. **`engines` is a hard gate under yarn classic** — a mismatch exits 1 with `The engine "node" is incompatible with this module`, it is not a warning. When bumping Node, bump all eleven in one sweep and keep the caret.
...
**Polyrepo setup: 11 independent indexes, not one.** The parent dir is a git repo but its `.gitignore`
excludes `/BEs/`, `/marketplace-admin/` and `/marketplace-shopowner/`, so indexing *here* sees only workspace docs. Each sub-repo
...
gitnexus group status marketplace-platform         # staleness across all 11
```
...
|---|---|---|
|the 11 indexed repos|`{"analyze": {"noStats": true}}`|generated block keeps its guidance, drops the volatile counts — byte-identical across runs|
...
| `gitnexus://group/marketplace-platform/status` | Staleness across all 11 repos |
