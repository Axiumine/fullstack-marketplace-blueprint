...
| MC-26 | every file that needs a shared environment value holds the same one — `KEYGRIP_KEK` across its seven holders, `REDIS_PASSWORD` and `REDIS_KEY` across their ten — and no repo shadows a shared key with a value only a shell without the direnv hook would read | `./scripts/audit-check.sh` §16 (blocking) | `./scripts/env-fingerprint-sweep.sh` over the real files, proved in both directions first by `./scripts/env-fingerprint-sweep-selftest.sh` — nine cases on a `mktemp -d` tree, never a real one |
...
⚠️ **`./scripts/audit-check.sh` exists because no test on this platform spans two repos.** Fifteen of its sixteen
