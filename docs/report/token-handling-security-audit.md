...
The comment defers the fix to an nginx rewrite **that does not exist**. All four checked-in configs under
`marketplace-user/docs/nginx/` were searched: zero `proxy_cookie_flags`, zero Secure directive. The only
hit for the string is `upgrade-insecure-requests` inside the CSP, which is unrelated. And per
`phase3/SECURITY_AUTH.md:270` those configs are documentation only — there is no `/etc/nginx` and no nginx
binary anywhere in this workspace, so even a correct directive would not currently be running.
...
- **ADR-018's "cookie scoped to API paths" is inaccurate prose, but root-scoping is load-bearing.** The
  ...
  catalogue requests (`cache.conf:21-30`). **Correct the sentence, not the scope.**
