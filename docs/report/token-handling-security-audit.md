...
2026-08-28, appends the cross-service-harness reason to §3.4's outcome — the accepted-residual
fact a now-retired record's own §7 held alone, carried here before that record was deleted and its content
distributed)
...
This report is amended in place rather than getting a v1.2 beside it, on the platform owner's decision,
...
> **Outcome — fixed, 2026-08-13.** `familyId` is stamped at login and carried unchanged
...
> (`src/others/sessionLifetime.mts:56`) is a retry, not an attack, and what the three SPAs
> actually do with several tabs open was measured. Confirmed live, not only by test: `report/live-auth-path-observation.md`.
...
> **Outcome — fixed, 2026-08-13.** `originalLogin` is stamped once at login and no
...
> **Outcome — fixed, 2026-08-13.** The index the fix said was missing
...
> no self-serve *log out everywhere* screen, and none was in scope here. A live defect found while auditing
> this was also fixed: `logout` matched on a field name that never existed, so it had never once
> ended a session.
...
> **Outcome — fixed, then the residual it left was fixed too, both 2026-08-13, the residual found during
> the live platform walkthrough.** The access token the call arrived with is deleted in the rotation pass, and so is the one the
...
> bound key at all.
>
> **How both halves of this finding's last acceptance criterion are proved, since no test drives two live services.** Three
...
> `sessionKeys.mts` being a single implementation already proves. That reasoning lived in a now-retired
> record's own §7 until it was deleted and distributed on 2026-08-28; this is where it now
> stands.
...
> **Outcome — fixed, and the fix outgrew the finding, 2026-08-11..13 (26 stories, landed in
> [`phase5/TELEMETRY_EGRESS_HARDENING.md`](../devprotocol/phase5/TELEMETRY_EGRESS_HARDENING.md)).** Certificate
> verification is on in all nine services and the `insecureHttpsModule` is gone;
> `sendDefaultPii` is `false` in all nine; neither can return by accident — an eslint rule and a
> per-service test refuse both shapes, and a Sentry major bump trips a named revisit.
> What the finding asked for as "a `beforeSend` hook stripping `Authorization` and `Cookie`" became
> `sentryBeforeSend`, which walks every bag on every event type and was verified against
> a real captured event and against real browser traffic. The audit's own §5 blind spot
...
- **a — fixed.** A session lives under `<REDIS_KEY><sha256('access:'+token)>`, built by
  ...
  at `<REDIS_KEY>dual-read-hits`, and that removing it was the one item in this backlog still held shut by
  a date rather than a decision. That removal landed on 2026-08-14 and deleted the fallback, the counter
  and `DUAL_READ_REMOVE_AFTER` together: the digest is now the only name a session has. Neither precondition
  ever had to be waited out — the platform has never been deployed, so no pre-cutover session existed
  anywhere, and the counter read zero on the one dev cluster.
- **b — fixed.** `<REDIS_KEY>idx:<tier>:<accountId>`, one field per
...
- **e — fixed by code, so the document became true.** `checkShopOwnerApproval`
  ...
  next refresh. `SECURITY_AUTH.md:80` describes a control that exists; that sentence was verified
  against the code rather than rewritten.
...
- **a — fixed.** `rememberMe` chooses `sessionCapDays` at login: unchecked is one day, checked is
...
- **b — documented, and the TLS half accepted with a trigger.** Redis persistence is
...
- **c — fixed (ADR-033).** `SameSite=Strict` is a named control with its own ADR and its own
  revisit triggers, so a `koa-utils` bump that relaxed it now trips something. The adjacent
  defect was found while removing dead variables: a `SAMESITE_COOKIE=lax` sat in seven `env` templates configuring
  nothing, describing the cookie wrongly.
- **d — fixed.** The docstring at `marketplace-common/src/others/resolveAuthorizationSession.mts`
...
> acted on exactly as instructed: **ADR-018's sentence was corrected, and no scope was touched**, and
...
- **No full dependency-tree audit** of `@axiumine/koa-utils` or `@sentry/node` beyond the specific
  mechanisms each finding needed. **Answered 2026-08-13**, in
...
- **Static only — answered.** The platform was brought up and driven:
  ...
  The twelve mechanical checks (MC-01..12) were written down next, so the next
  audit is a command rather than a reading.
- **Application/access logging — answered, and acted on.** Measured rather than assumed, in
  [`architecture.md`](../architecture.md) §*What the logs actually contain*. What the measurement found
  was fixed as part of the same telemetry and egress hardening work documented in
  [`phase5/TELEMETRY_EGRESS_HARDENING.md`](../devprotocol/phase5/TELEMETRY_EGRESS_HARDENING.md): no resolver
  echoes its argument, the edge records no client address, every container log is bounded and its retention
  is pinned by the repo.
- **Rate limiting on `refresh` — answered.** It was not limited; it is now, keyed on the session's
  own lineage rather than on a client IP, which is the constraint
  [`phase5/TELEMETRY_EGRESS_HARDENING.md`](../devprotocol/phase5/TELEMETRY_EGRESS_HARDENING.md) had already
  imposed. **The residual this leaves is named, not hidden:** a distinct-token flood still reaches Redis once
  per token, carried as a risk-register row rather than as a claim of completeness.
- **Multi-tab behaviour — answered.** Measured across the three SPAs before the grace window was
...
> the key-digest cutover ran before §3.3 and §3.4, because both the account→sessions index and the
> rotation-family sets are built on the hashed keyspace, and doing it afterwards would have meant migrating
> them twice. The deviation is recorded in [`phase3/SECURITY_AUTH.md`](../devprotocol/phase3/SECURITY_AUTH.md) §3.6. The closing promise held too: **no coverage
> or mutation threshold was lowered anywhere in this backlog, and no gate was removed.** Two were added — a
> boundary contract and a dependency-advisory scan that actually reports.

---

## 7. Closing record — what happened to every finding

Written 2026-08-13, when the backlog this report opened had landed all but three of
its stories. Fifteen findings, none unassigned. "Fixed" means the mechanism the finding asked for exists
...
|---|---|---|---|
| 3.1 | Rotation without reuse detection | fixed | `newSessionLineage.mts`, `refreshSessionTokens.mts`, `revokeSessionFamily.mts` |
| 3.2 | No absolute session lifetime | fixed | `sessionLifetime.mts:20,35` |
| 3.3 | No credential write invalidates a session | fixed | `revokeAllSessionsForAccount.mts:79` |
| 3.4 | The old access token survives a refresh | fixed | `refreshSessionTokens.mts:250-255`; header-less path closed 2026-08-13 by the `accessKey` field, `live-auth-path-observation.md` §6; the revocation residual **R54** closed the same day, `sessionKeys.mts` `retireAccessSession`. Both halves of the last acceptance criterion are proved by three real-cluster assertions across two suites rather than by one two-service test — §3.4's outcome block names them |
...
| 3.7b | Redis persistence and Redis-protocol TLS undocumented | documented; TLS **accepted** | **R45**, `redisScheme.test.mts` is its trigger |
| 3.7c | `SameSite=Strict` named in no ADR | fixed | **ADR-033** |
| 3.7d | Stale docstring on key-namespace safety | fixed | `resolveAuthorizationSession.mts` |

§4's five do-not-fix items all held, and §5's five blind spots were all answered — three by investigations
this backlog ran (the application/access-log investigation, the multi-tab measurement and the live platform
walkthrough) and two by reports written for the purpose (the encryption-at-rest coverage report and the
dependency-tree advisory scan).

**The items this report does not close on its own, and where each of them stands:**

- **The pre-cutover dual-read fallback and its counter** — **closed 2026-08-14.** It was
  ...
  became one key, and the integration test for that shape was **inverted** rather than deleted: it still seeds a
...
- **Revocation ending refresh sessions only** — **closed 2026-08-13, hours after it was written
  here.** `revokeAllSessionsForAccount` and the admin session console's `funRevokeSession` retire the access half
