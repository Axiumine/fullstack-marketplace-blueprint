...
| 4 | ~~Two independent writers of `item.published`~~ **Closed 2026-08-14 — accepted, last writer wins** | ⚠️ **The platform owner's decision, taken with the same race on `company`: an operator unpublishes, the owner publishes it again, and that is fine.** No version or lock field is added to `item.js`. Two things the decision now covers, read off the code rather than assumed. **The two writers are not symmetric:** `funItemUpdatePublished` writes `{ $set: { published } }` and nothing else, while the owner's `funItemUpdate` `$set`s the whole card and `IItemUpdate` keeps `published` — so an owner's ordinary save of an unrelated field restores their own value of the flag, without touching it and without meaning to. **And nothing records that an operator flipped it:** `funItemUpdatePublished`'s docblock already called this "a takedown that does not stick", held open on a policy decision nobody had taken. It has now been taken. The takedown that does stick is `itemDel` — `deleted` is outside `IItemUpdate`, so no owner save revives a soft-deleted item. |
