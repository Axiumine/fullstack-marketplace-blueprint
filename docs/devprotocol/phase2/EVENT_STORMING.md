...
v1.12 - 2026-08-30: **§2's customer aggregate gains the two Close Account commands and the event they emit** — the customer's own close, shipped 2026-08-26, and the Admin tier's, built today ([ADR-048](../phase3/adr/ADR-048-an-admin-closes-a-customer-account.md)). The section had said no mutation anywhere sets `deleted` on a `user`, which the first of the two had already made false. One event, two actors, and `deletedBy` is what tells them apart.
v1.10 - 2026-08-27, later the same day: §6 question 6 cited `funCompanyAdd.mts:34-36` for a guard that is now at `:39-41` — the lines moved down when a comment block was written above them, and the code is unchanged. The row also records that `BOUNDED_CONTEXT.md` §7 question 6 said `Open` until today, thirteen days after this closure. Four citations pointed at "§5 open question 4" for a row that has always been in §6 — §5 is the hotspot table — and all four are corrected in the same pass: two in `phase3/adr/ADR-038…` (which records the correction in a note of its own), one in `BOUNDED_CONTEXT.md` §7 q4 and one in this changelog's own v1.9 entry. No command, event, actor, aggregate or hotspot changed.
...
`user` aggregate was the customer's own. `userUpdateStatus` was built as part of the admin active-user work
(ADR-049); its command, its two outcomes and
...
v1.1 - 2026-08-12: The self-service registration §2.2 recorded as absent was built — *A seller registers
themselves and waits for an admin* (`phase5/SHOPOWNER_ONBOARDING_APPROVAL.md` §4). That flow, its
...
v1.2 - 2026-08-13: **hotspot 2 and §7 question 2 close on a decision, not on a build** (*Admin sets
onboarding and session preferences*, `phase5/SHOPOWNER_ONBOARDING_APPROVAL.md` §4). The
...
`phase4/DDD_AGGREGATES.md` §10 q3 and `phase5/SHOPOWNER_ONBOARDING_APPROVAL.md` §6 q4
close with it. Nothing survives as a residual.
...
Admin          →   Set Status (userUpdateStatus)           →   Customer Suspended (disabled→true)
                                                            ...
                                                            →   Every Session Of That Customer Destroyed
                                                                (on suspension only)
Customer       →   Close Account (userDel)                 →   Customer Account Closed (deleted stamped,
                                                                deletedBy absent — the holder did it)
                                                            →   Close Refused — Already Closed (410)
                                                            →   Every Session Of That Customer Destroyed
Admin          →   Close Account (userDel, Admin tier)      →   Customer Account Closed (deleted +
                                                                deletedBy — an admin did it)
                                                            →   Close Refused — Not Found (404)
                                                            →   Every Session Of That Customer Destroyed
```

`userUpdateStatus` is the admin's lever on a registered customer and one of the two commands on this
aggregate the customer cannot issue — the other is the Admin tier's `userDel`, 2026-08-30 — Admin tier, `BEs/dev/marketplace-dev-admin-authenticated-resource/src/graphQLApi/schema/mutations/userUpdateStatus.mts` (admin active-user work, ADR-049, 2026-08-25). It writes `disabled` and nothing else. **Closing an account is a different command with a different event**: `userDel` on the customer tier stamps `deleted` (ADR-036), and the Admin tier's `userDel` stamps `deleted` and `deletedBy` ([ADR-048](../phase3/adr/ADR-048-an-admin-closes-a-customer-account.md)). ⚠️ **`userUpdateStatus` writes no `deleted*` field and neither closure writes a `disabled*` one**, so a suspension cannot be laundered into a closure or back. The event is *Customer Account Closed* rather than "deleted" — nothing is removed (ADR-041) — and `deletedBy` says which actor emitted it. Restoring emits no session event — nobody's credentials changed, and signing a customer out for being re-enabled is not a control.
...
the 2026-08-10 token-handling audit (`report/token-handling-security-audit.md` §3.6) a session lives under the digest of it, and `readSessionField` is what tries the digest first and
the raw token second while the pre-cutover sessions drain. And the refresh field was `id` until the session-revocation work (`phase5/SESSION_TERMINATION.md` §3.1) —
`id` being a name no writer has ever written: **"Session Destroyed" was an event this platform announced and did not
...
`onboardingStep` / `onboardingDone` are read, not written, at every auth-middleware site found: `BEs/dev/marketplace-dev-authenticated-authorization/src/lib/auth/tokenInfoShopOwner.mts`, `.../src/lib/auth/authenticatedAuthorizationHandler.mts`, `BEs/dev/marketplace-dev-authenticated-resource/src/lib/auth/makeAuthCtx.mts`. The one writer is `shopOwnerUpdatePreferences` on the Admin service — an admin typing a step in by hand. Nothing advances either field as a side effect of a shop owner doing anything, which is what §5 hotspot 2 was about and what *A seller registers themselves and waits for an admin* (`phase5/SHOPOWNER_ONBOARDING_APPROVAL.md` §4) made urgent: an approved self-registered account arrives with a login and nothing else, so onboarding is now the flow between an approval and a shop. ⚠️ **Settled 2026-08-13, *Admin sets onboarding and session preferences* (same record, §4): the admin's hand is the writer, deliberately, until that flow is designed** — hotspot 2 and §7 question 2 close on the decision, and the work itself is deferred rather than done.
...
|---|---|---|---|
| 1 | Does self-service shop-owner registration ever get built, or does Admin-provisioning stay permanent? | Product | **Closed 2026-08-12 — *A seller registers themselves and waits for an admin* (`phase5/SHOPOWNER_ONBOARDING_APPROVAL.md` §4) — built, and the two coexist.** |
| 2 | What advances `onboardingStep`, and where does that write live? Nothing but an admin's hand, today — see hotspot 2 | Platform dev | **Closed 2026-08-13 — *Admin sets onboarding and session preferences* (`phase5/SHOPOWNER_ONBOARDING_APPROVAL.md` §4) — an admin's hand, and that is the decision until a shop-owner onboarding flow is designed. Deferred, not built; residual R53.** |
| 3 | Is `waitApprov`'s state at account creation "approved" or "pending" by default? | Platform dev | **Closed 2026-08-12 — *A seller registers themselves and waits for an admin* (`phase5/SHOPOWNER_ONBOARDING_APPROVAL.md` §4) — neither is a default: pending when the seller registered themselves, approved when an admin created them.** |
| 4 | When order/cart/payment/delivery design work starts, who signs off the first schema? | Platform owner | **Closed 2026-08-27 — moot: design work does not start.** The four are permanently out of scope ([`ADR-038`](../phase3/adr/ADR-038-commerce-is-permanently-out-of-scope.md)), so there is no first schema. ⚠️ The owner cell read **Product + platform dev** for the life of the question and that was the tell — this platform has no product function; it is a blueprint published for the community (ADR-037) and the platform owner decides everything here. Same closure as `BOUNDED_CONTEXT.md` §7 q4 and `phase1/PDR.md` open question 3, and the five open questions ADR-038's closing note now folds in. |
