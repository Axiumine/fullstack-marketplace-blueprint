...
v1.11 - 2026-08-27, later still: the epic file that recorded E11 is deleted; the epic id E11 stays in the numbering, per ADR-038 §Consequences, which only the file dies. §6 open question 4's closure row pointed at all five of that file's §6 open questions — it now points at ADR-038's own §Note, which is where that record now lives. v1.10's citation of the same now-deleted file, below, is annotated in place rather than rewritten. Nothing else in this document — no command, event, actor, aggregate, hotspot or open question's substance — changed; the E11-side detail that question 4 used to carry stays with the ADR, not here.
v1.10 - 2026-08-27, later the same day: §6 question 6 cited `funCompanyAdd.mts:34-36` for a guard that is now at `:39-41` — the lines moved down when a comment block was written above them, and the code is unchanged. The row also records that `BOUNDED_CONTEXT.md` §7 question 6 said `Open` until today, thirteen days after this closure. Five citations pointed at "§5 open question 4" for a row that has always been in §6 — §5 is the hotspot table — and all five are corrected in the same pass: two in `phase3/adr/ADR-038…` (which records the correction in a note of its own), one in the epic file that recorded E11 (⚠️ that file was deleted 2026-08-27, see v1.11 below — the correction itself moved with it to ADR-038's note), one in `BOUNDED_CONTEXT.md` §7 q4 and one in this changelog's own v1.9 entry. No command, event, actor, aggregate or hotspot changed.
...
out of its epic the same day — close with it. Nothing survives as a residual.
...
aggregate the customer cannot issue — the other is the Admin tier's `userDel`, 2026-08-30 — Admin tier, `BEs/dev/marketplace-dev-admin-authenticated-resource/src/graphQLApi/schema/mutations/userUpdateStatus.mts` (E19, 2026-08-25). It writes `disabled` and nothing else. **Closing an account is a different command with a different event**: `userDel` on the customer tier stamps `deleted` (ADR-036), and the Admin tier's `userDel` stamps `deleted` and `deletedBy` ([ADR-048](../phase3/adr/ADR-048-an-admin-closes-a-customer-account.md)). ⚠️ **`userUpdateStatus` writes no `deleted*` field and neither closure writes a `disabled*` one**, so a suspension cannot be laundered into a closure or back. The event is *Customer Account Closed* rather than "deleted" — nothing is removed (ADR-041) — and `deletedBy` says which actor emitted it. Restoring emits no session event — nobody's credentials changed, and signing a customer out for being re-enabled is not a control.
