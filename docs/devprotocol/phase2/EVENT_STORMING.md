...
v1.12 - 2026-08-30: **§2's customer aggregate said no mutation anywhere sets `deleted` on a `user`, so "Customer Account Closed" was not an event this platform emits.** It stopped being true on 2026-08-26, when `userDel` shipped on the customer tier, and the sentence outlived it by four days. Two commands are added and the sentence is struck: the customer's own close, and the Admin tier's, built 2026-08-30 ([ADR-048](../phase3/adr/ADR-048-an-admin-closes-a-customer-account.md)). One event, two actors, and `deletedBy` is what tells them apart.
...
aggregate the customer cannot issue — the other is the Admin tier's `userDel`, 2026-08-30 — Admin tier, `BEs/dev/marketplace-dev-admin-authenticated-resource/src/graphQLApi/schema/mutations/userUpdateStatus.mts` (E19, 2026-08-25). It writes `disabled` and nothing else — ~~**no mutation anywhere sets `deleted` on a `user`**, so "Customer Deleted" is not an event this platform emits~~. ⚠️ **Both halves are false and were superseded four days apart.** `userDel` on the customer tier writes `deleted` from 2026-08-26 (ADR-036), and the Admin tier's `userDel` writes `deleted` and `deletedBy` from 2026-08-30 ([ADR-048](../phase3/adr/ADR-048-an-admin-closes-a-customer-account.md)). What survives the correction is the separation the sentence was reaching for: **`userUpdateStatus` still writes no `deleted*` field and neither closure writes a `disabled*` one**, so a suspension cannot be laundered into a closure or back. The event is emitted, it is *Customer Account Closed* rather than "deleted" — nothing is removed (ADR-041) — and one field says which actor emitted it. Restoring emits no session event — nobody's credentials changed, and signing a customer out for being re-enabled is not a control.
