...
| 3 | Is `waitApprov` (BC-03) true or false/absent by default at account creation? Schema comment conflates "awaiting approval" with "deleted" in one field's own doc comment (`BEs/marketplace-db-setup/lib/schemas/shopOwner.js`). | Platform dev | Open - **and load-bearing since q8 closed**: on disk `shopOwnerAdd` writes no `waitApprov`, so an account is born ungated and the gate only ever bites an account an operator has parked by hand. If it should be the other way round, it is one line in `shopOwnerAdd` plus a migration for the existing rows, not a lint rule |
