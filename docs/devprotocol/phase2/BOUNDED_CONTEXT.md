...
v1.17 - 2026-08-27, later the same day: `phase5/epics/E11.md` is deleted and its record is absorbed elsewhere —
ADR-038's own closing note and `phase5/EPICS_STORIES.md` §6.1 are its two new homes. Every citation of that file
...
distinction and the concern-vs-context fork that only `E11.md` had recorded; both were already moot. No context,
...
without touching the flag. `phase5/epics/E11.md` §6 q3 stayed open on purpose at the time — it was about
an order referencing a live flag, which was BC-11 design work, not this race — until it closed too, as
**moot**, on 2026-08-27 ([`ADR-038`](../phase3/adr/ADR-038-commerce-is-permanently-out-of-scope.md)); that
file does not exist any more, and §7 q5 below carries the closure now.
...
**Produces:** the compiled `@axiumine/marketplace-common` package, published to `registry.npmjs.org` at `2.0.0` with every consumer on `^2.0.0` (`ADR-037`), and synced into every consumer's `node_modules` by `BEs/marketplace-common/deploy-local.sh` - which is what makes an edit visible *before* it is released, and which a plain `yarn install` undoes.
...
| BC-10 Shared Kernel package boundary | An edit to `marketplace-common` is invisible to every consumer until synced - the registry serves what was released, not what was edited. ⚠️ **Restated 2026-08-27:** this cell read *"the package is not on any registry (`@axiumine/marketplace-common` 404s on `registry.npmjs.org`)"*; it is published — `1.0.1` that day, `2.0.0` since 2026-08-27 (`phase3/adr/ADR-037-marketplace-common-is-published-to-npm.md`), and a `yarn install` restores the last released build over a deployed one — which only loses something while common carries an unreleased edit, and which no install path may be wired to prevent | `BEs/marketplace-common/deploy-local.sh` (must be re-run after every edit; never invoked by an install), `yarn test:contract` (verifies the `exports` map against actual files) |
