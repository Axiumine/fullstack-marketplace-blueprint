...
are restated on the release. The three earlier entries stand as the record of what was true when they
were written. No context, boundary, aggregate or relationship changed.
...
registry"* and that `deploy-local.sh` is *"the only thing that makes an edit visible"*. `ADR-037` published it
on 2026-08-26; the script now bridges
...
v1.10 - 2026-08-25: BC-07's "no lever exists after registration either" was true for one day. The admin active-user-table work (`phase3/adr/ADR-041`, `ADR-046`, `ADR-049`) built `userUpdateStatus`, `usersActiveTbl` and the `/customers` screen the same day the gap was written down, so the paragraph names the writer and the table instead of the hand-made MongoDB write. The approval-gate half of the paragraph is unchanged and still permanent.
v1.9 - 2026-08-25: BC-07 gains the answer to the question `phase5/CUSTOMER_ACCOUNT_ADDRESSES.md` §6 said it did not carry — a customer tier with no approval gate is the permanent design, not a starting point, and `user.disabled` is read by every gate and written by nothing.
...
BC-12 is **withdrawn** and is not coming: it was proposed only to give the keygrip-custody design (`phase3/adr/ADR-034-keygrip-keys-live-in-redis-wrapped-under-a-kek.md`) and the admin session console (`docs/testing.md`, `docs/decisions/admin-session-tooling-placement.md`) a context to own, under a rule that `phase5/CONSTRAINTS.md` §5 has since removed. The eleven contexts themselves are
unchanged.
...
*The two admin-only `shopOwner` fields are refused by construction* (`phase5/IDENTITY_ACCESS.md`) / CON-12, the `user` one has nothing to enforce yet and says why. Question 8 opened in the same
...
v1.4 - 2026-08-12: questions 1 and 3 both closed by *A seller registers themselves and waits for an admin* (`phase5/SHOPOWNER_ONBOARDING_APPROVAL.md`), which built the public seller registration.
...
close in meaning and absent in fact, found while checking that BC-11 and the file that recorded its gap
agreed on the same four named concepts. Nothing else in BC-11 moved: it owns nothing, produces nothing and is still the one context this
platform may not design without being asked.
v1.6 - 2026-08-13: BC-03's last hotspot and §7 question 2 close (*Admin sets onboarding and session preferences*, `phase5/SHOPOWNER_ONBOARDING_APPROVAL.md`), and both closed on a **factual
...
documents existed. The claim that story makes, and the only one that is true, keeps the word this file
...
not an extension of an existing context.
...
**Consumes:** bcrypt-hashed credentials from each collection (`SALT_ROUNDS=14`), the Keygrip-signed refresh cookie, and the `Authorization: Bearer access:` header — the only two credentials this context reads, with nothing else standing in for either (`resolveAuthorizationSession`, `BEs/marketplace-common/src/others/resolveAuthorizationSession.mts`).
