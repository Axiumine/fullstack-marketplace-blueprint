...
| 2 | ~~What advances `shopOwner.onboardingStep`/`onboardingDone` (BC-01/BC-03 boundary), and where does that write live?~~ ⚠️ The second sentence was wrong and is deleted: `shopOwnerUpdatePreferences` (Admin tier) writes both fields and always has. | Platform dev | **Closed 2026-08-13 (E03-S04) — an admin's hand, by decision. A shop-owner-side write is deferred design, not a missing line; residual R53.** |
