...
| CON-09 | `marketplace-common` consumed by package name (`@axiumine/marketplace-common`), published on npmjs at `2.0.0`, consumers on `^2.0.0`. `deploy-local.sh` is what makes an *unreleased* edit visible to the 9 consumers, and what a `yarn install` silently undoes — in that direction only: no install path in any of the 16 repos invokes the script, and none may. ⚠️ **Restated 2026-08-27:** this row read *"not on any registry (404s npmjs)"* and its trigger treated a real publish as out of scope — `ADR-037` decided it on 2026-08-26. | `docs/workflow.md` §Repo layout; `phase2/BOUNDED_CONTEXT.md` §6 Anti-corruption layers, BC-10 row; `phase3/adr/ADR-037-marketplace-common-is-published-to-npm.md` | ADR assumes `yarn install` alone picks up an *unreleased* common change, assumes the package is unpublished, or wires the script into an install path |
