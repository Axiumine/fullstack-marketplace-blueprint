...
| `defaultAddress` is single top-level ObjectId pointer, not per-element boolean | [`docs/data-model.md`](../../data-model.md) `user` divergence #4 — pointer makes "2 defaults" inexpressible, DB `$expr` enforces validity; already paid for, not up for re-litigation |
