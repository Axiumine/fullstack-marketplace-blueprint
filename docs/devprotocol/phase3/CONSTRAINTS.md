...
v1.10 - 2026-08-30: **CON-09 restated again, and its short name changes.** The platform owner ruled that an
edit to `marketplace-common` reaches a consumer by being published and by nothing else, and deleted the
sync script ([`adr/ADR-047-a-common-change-ships-as-a-published-release.md`](./adr/ADR-047-a-common-change-ships-as-a-published-release.md)).
The row loses the bridge half that v1.6 and v1.7 spent two revisions narrowing — with nothing hand-placed
in a consumer's `node_modules`, a `yarn install` restores what the lockfile names and undoes nothing — and
the trigger gains the shape that is now the violation: any ADR that reintroduces a local copy under any
name. Versions move to `3.0.0` / `^3.0.0`. The constraint itself is unchanged: a consumer compiles against
a version, not against an edit.
v1.9 - 2026-08-27, later the same day: CON-09's version string follows the release of `marketplace-common`
`2.0.0` — published on npmjs, consumers on `^2.0.0`. The constraint itself is untouched.
...
v1.3 - 2026-08-12: following *a seller registers themselves and waits for an admin*, CON-12 records that `marketplace-dev-public-resource` is the one repo the
...
v1.2 - 2026-08-12: CON-12 added — admin-only fields are listed and linted, not translated. Records the
closed answer to a question three phase documents had open: no anti-corruption layer across `shopOwner` or
`user`, now or later.
v1.7 - 2026-08-27, later the same day: CON-09's trigger read "ADR assumes `yarn install` alone picks up a common change", which is now true of a *released* change and false only of an unreleased one. The trigger is narrowed to the unreleased case, and the row states that no install path invokes `deploy-local.sh`. The constraint is unchanged.
v1.6 - 2026-08-27: **CON-09 restated.** It constrained the platform on the premise that `marketplace-common` is *"not on any registry (404s npmjs)"*, and its trigger fired on any ADR proposing a real publish — which `ADR-037` then did, on 2026-08-26. The constraint survives with its premise replaced: the package is published at `1.0.1`, `deploy-local.sh` carries what no release has shipped, and ⚠️ a plain `yarn install` silently undoes it. Trigger now fires on an ADR assuming the package is unpublished. No constraint added or removed.
...
| CON-08 | 100% coverage all 4 metrics + 100 mutation score, every package that ships code (9 BE services, `marketplace-common`, `marketplace-db-setup`, 3 frontends, `marketplace-services-status`). Never lower threshold, never remove gate. | `CLAUDE.md` §Rules that apply to every task, `README.md` §Test quality gates; `phase1/PDR.md` §Key outcomes, §6; `phase1/NFR.md` §3 NFR-MA01/MA02/MA05 | ADR proposes lowering a threshold, skipping a gate, or `ignoreStatic` to silence a Stryker survivor |
| CON-09 | `marketplace-common` consumed by package name (`@axiumine/marketplace-common`), published on npmjs at `3.0.0`, consumers on `^3.0.0`. **Publishing is the only route in**: an edit no release has shipped reaches no call site, and a `yarn install` is authoritative everywhere because there is no hand-placed build for it to overwrite. ⚠️ **Restated twice.** 2026-08-27: this row read *"not on any registry (404s npmjs)"* and its trigger treated a real publish as out of scope — `ADR-037` decided it on 2026-08-26. 2026-08-30: the local sync script it named is deleted (`ADR-047`). | `docs/workflow.md` §Repo layout; `phase2/BOUNDED_CONTEXT.md` §6 Anti-corruption layers, BC-10 row; `phase3/adr/ADR-037-marketplace-common-is-published-to-npm.md`; `phase3/adr/ADR-047-a-common-change-ships-as-a-published-release.md` | ADR assumes `yarn install` alone picks up an *unreleased* common change, assumes the package is unpublished, or reintroduces a local copy into a consumer's `node_modules` under any name — a script, `yarn link`, a `file:` path |
