...
name. The constraint itself is unchanged: a consumer compiles against
...
| CON-06 | 3 `*-authenticated-authorization` services share body via `marketplace-common` (`resolveAuthorizationSession`, `findAccountForSession`, `refreshSessionTokens`) but stay 3 deployables, 3 ports (4025/4029/4031). Merge into 1 process decided AGAINST 2026-08-07. | `docs/architecture.md` §Auth model; `docs/decisions/authorization-service-consolidation.md`; `phase1/PDR.md` §Out of scope, `phase1/NFR.md` NFR-AV01/AV02 | ADR re-opens "merge the 3 authz services into 1", cites dedup as reason |
...
| CON-09 | `marketplace-common` consumed by package name (`@axiumine/marketplace-common`), published on npmjs, consumers on the range each declares. **Publishing is the only route in**: an edit no release has shipped reaches no call site, and a `yarn install` is authoritative everywhere because there is no hand-placed build for it to overwrite. | `docs/workflow.md` §Repo layout; `phase2/BOUNDED_CONTEXT.md` §6 Anti-corruption layers, BC-10 row; `phase3/adr/ADR-037-marketplace-common-is-published-to-npm.md`; `phase3/adr/ADR-047-a-common-change-ships-as-a-published-release.md` | ADR assumes `yarn install` alone picks up an *unreleased* common change, assumes the package is unpublished, or reintroduces a local copy into a consumer's `node_modules` under any name — a script, `yarn link`, a `file:` path |
