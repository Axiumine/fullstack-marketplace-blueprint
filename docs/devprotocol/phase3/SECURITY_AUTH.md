...
| 2 | Whether a freshly created `ShopOwner` starts `waitApprov`-gated or ungated — `shopOwnerAdd` sets no initial value, so it starts ungated | platform owner | open, and load-bearing since the gate went live — `phase2/BOUNDED_CONTEXT.md` BC-03 hotspot 1 and §7 q3 |
