...
| ShopOwner logging in before manual approval | `waitApprov` read at login | `BEs/marketplace-db-setup/lib/schemas/shopOwner.js` | **Open, unresolved on disk**: whether a freshly created `ShopOwner` starts gated or ungated — `phase2/BOUNDED_CONTEXT.md` BC-03 hotspot 1, no mutation confirmed to set the initial value |
