...
No bootstrap script generates `.env` files on this platform — each repo's untracked `.env` is populated by hand per machine, against the committed `env` template's key list. Nothing here auto-provisions a Keygrip pair or an `INTROSPECTION_CODE`; getting two related services' copies to agree is a manual, currently unverified-by-tooling step (§3.6, §2). This is a gap: **the enforcement layers in §5 stop a secret leaving the machine, but nothing stops two machines' secrets from silently disagreeing.**

Three enforcement layers on the write/leak side, all in `.claude/SECRETS.md`:
...
### nginx security surface — documentation only

`marketplace-user/docs/nginx/{cache.conf,marketplace-user.conf,rate-limit.conf,security-headers.conf}` are real, deployable configs. **They are documentation until someone installs them** — there is no `/etc/nginx` directory and no `nginx` binary in `PATH` on this machine or anywhere in this workspace. Every claim below is what the config *specifies*, not a running control.

Upstream map, all loopback, matching the port table in `docs/architecture.md` §Services:
...
**TLS + HSTS + response headers** (`security-headers.conf:16-21`):
...
Plus a `Content-Security-Policy` (`security-headers.conf:26+`). A load-bearing nginx trap recorded in the file itself: `add_header` does **not** merge — any `location` block declaring even one `add_header` of its own discards every header inherited from the `server` block, so every location that needs these headers must repeat them, not assume inheritance.

**Auth-path rate-limit zones**, edge layer independent of the application-level limiter in §3.7 (`rate-limit.conf:4,11-26`):
...
**Cache bypass on session cookie** — the nginx half of the §3.8 mechanism, `cache.conf:21-30` (quoted there in full). PMTiles range requests and the Nominatim proxy topology are documented in `phase1/SYSTEM_CONTEXT.md` §5.8-5.9 and are out of Phase-3 auth scope.

Two gaps surfaced by cross-checking the docs against the actual `marketplace-user` tree, both already logged as open questions in `phase1/SYSTEM_CONTEXT.md` §7 and repeated here because they are security-relevant: `marketplace-user.conf:173-176` documents a proxied `/api/register` SSR route that does not exist in `src/routes/` (actual registration is client → GraphQL `userRegister` straight to `/public-resource`, guarded by `assertUnderRateLimit`/Turnstile in the resource service itself — the doc is either stale or the route was never built); and no admin-facing vhost (port 4025) appears in any file under `marketplace-user/docs/nginx/`, so whether an equivalent exists for `marketplace-admin`/`marketplace-shopowner` is unconfirmed.
...
| 4 | Is `marketplace-user.conf:173-176`'s documented `/api/register` SSR proxy route stale doc, or an unbuilt route? | platform owner | open — `phase1/SYSTEM_CONTEXT.md` §7 item 1 |
| 5 | Does an admin-facing nginx vhost exist for `marketplace-admin`/`marketplace-shopowner` (port 4025 absent from all files under `marketplace-user/docs/nginx/`)? | platform owner / ops | open — `phase1/SYSTEM_CONTEXT.md` §7 item 2 |
