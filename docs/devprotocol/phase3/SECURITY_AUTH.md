...
| Value left **unquoted** and read differently by each of the two env readers | `.githooks/pre-commit` check 0 since 2026-09-01 — an unquoted value holding whitespace or a `#` is blocked | [`.claude/SECRETS.md`](../../../.claude/SECRETS.md) §3; [`docs/workflow.md`](../../workflow.md) §Environment files; RISK_REGISTER R05 | ⚠️ The mechanism this row carried does not exist: dotenv 17.4.2 reads `KEY=abc def` whole and cuts nothing at a space, measured 2026-09-01. What is real is narrower — `KEY=abc#tail` is `abc` to both readers and neither says so, and an unquoted space is an `invalid line` to the direnv 2.32.1 builtin over the shared layer, which answers by exporting **none** of that file. The 2026-08-07 truncation this row claimed, 89 chars to 76, is the wrap of the row below it. Residual: `--no-verify`, and merge commits, do not fire it |
| Secret value silently truncated by a **line break inside a quoted value** | `.githooks/pre-commit` check 0 — quoting does not help here, the newline ends the value regardless | [`.claude/SECRETS.md`](../../../.claude/SECRETS.md) §3; [`docs/workflow.md`](../../workflow.md) §Environment files; RISK_REGISTER R05b | 2026-08-09: all 5 `.env` files holding `KEYGRIP_KEY_1`/`_2` had both 88-char keys wrapped after 76 chars, the 12-char tail sitting on the next line with no `KEY=` prefix, so dotenv exported a truncated key **and** a junk variable named after the tail. The user-tier authz file had the tail duplicated a second time. Repaired by rejoining; all 5 files now fingerprint-identical. The detector — a non-blank, non-comment line that does not match `^[A-Z][A-Z0-9_]*=`, a value opening a quote the line never closes, or an unquoted value opening with the `=` of base64 padding — now **runs as pre-commit check 0** in all 16 repos, reading the working tree because a `.env` is never staged. ⚠️ Re-measured 2026-09-02: dotenv 17.4.2 keeps a quoted wrap as one newline-bearing value rather than truncating it, and only an unquoted wrap is cut short - the orphan tail becomes a variable when it carries an `=`. direnv 2.32.1 refuses either shape and exports nothing from the file at all. Residual: `--no-verify`, and merge commits, do not fire it |
...
3. **`pre-commit` guard**, tracked at `.githooks/pre-commit` in all 16 repos — `marketplace-nginx` was the last to get one and, having no `package.json`, carries this guard and no gate after it (ADR-030) — opens with **check 0**, the only check on the platform that reads the *working tree* rather than the staged index (the file it exists for is git-ignored and never staged): it blocks when that repo's `.env`, `.env.*`, `env` or `env.shared` holds a non-blank, non-comment line that is not `KEY=VALUE`, or a value opening a quote the line never closes — the two signatures of one value broken over two physical lines (§2, R05b) — and, since 2026-09-01, an unquoted value holding whitespace or a `#`, which dotenv and direnv do not read the same way (§2, R05). It prints file, line and key name, never a value. Then the two staged-secret scans: blocks a commit whose staged path looks like a secret file, or whose staged added lines contain a high-entropy pattern (npm token, PEM header, 40+ char non-placeholder `KEYGRIP_KEY_*`, a `QODANA_TOKEN`/`SOCKETLABS_*`/`REDIS_PASSWORD` literal ≥16 chars, a Mongo URI with a ≥10 char password, and since 2026-09-06 a credential passed as a command-line flag — `-password`, `--pass`, `--pwd` — whose value is 12 characters or more and does not open with `$`, `x` or `<`). A self-declared placeholder line (`KEY=test-…`, `dummy`/`fake`/`sample`/`example`/`changeme`) is exempted so committed templates and vitest fixtures pass without `--no-verify`. Escape hatch is `git commit --no-verify`, which is a gap by design — nothing prevents a developer from using it.
...
⚠️ **Corrected 2026-09-06 — this paragraph described a state that ended on 2026-08-08 and nobody updated it.** `BEs/marketplace-db-setup/setup/mongodb.js` is **not tracked**: it and `setup/redis.txt` are listed in that repo's `.gitignore`, whose own comment says why — *the remote is public, a tracked copy is one push away from publishing every credential in it*. The purge is done too. The repository's twenty pre-2026-08-08 commits were collapsed into a single root commit (`ee897c9`, *chore: collapse the history into a single commit*) precisely because they carried those credentials, and the blobs are gone: `git cat-file --batch-all-objects` over the whole object database — reachable and dangling alike — finds none of them, and `marketplace-db-setup` has never been pushed, so no remote ever held them either. `./scripts/history-scan.sh` is that search, kept as a script so it is repeatable rather than a claim somebody once made, and `./scripts/audit-check.sh` §9 runs it over all sixteen (`docs/testing.md` MC-17). The guard above also stopped missing the shape this file was written in: since 2026-09-06 all sixteen hooks carry a rule for a credential passed as a command-line flag — `mongosh … -password <the value>` — which no `KEY=`-anchored or URI rule could ever see (MC-16).

⚠️ **What is still open is the credential itself, and it is not an engineering task.** The value in the gitignored runbook is live and has never been rotated, and nothing readable from this workspace can rule out a clone or a backup taken before the squash on a machine nobody here can see. That is RISK_REGISTER **R57**, and it belongs to the platform owner.
