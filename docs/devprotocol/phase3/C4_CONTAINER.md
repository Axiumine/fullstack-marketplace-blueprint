...
v1.9 - 2026-08-30, later the same day: the 4024 row gains `userDel` ([ADR-048](./adr/ADR-048-an-admin-closes-a-customer-account.md)). It had said the Admin service could *delete* nothing on `user`; the rest of that sentence — no create, no edit of a customer's data — stands. No container, edge or port moves: it is one more mutation on a service already drawn.
...
| `marketplace-dev-admin-authenticated-resource` | 4024 | Admin | Koa 3 + Apollo Server 5, `ENDPOINT = '/admin-authenticated-resource'` | Domain data for Admin — `shopOwnerAdd`/`shopOwnerUpdateStatus` (approval), **sole writer** of `itemCategory` (`funItemCategoryAdd.mts` enforces the depth-2 cap), moderation (`companyDel` any company, `itemUpdatePublished`/`itemDel`). Since 2026-08-25 also the admin's only reach into `user`: `usersActiveTbl` pages customer accounts and `userUpdateStatus` suspends or restores one, ending every session of the suspended account through `endEveryUserSession` ([ADR-049](./adr/ADR-049-the-admin-tables-offer-the-four-account-states.md)). ⚠️ **This is the only service that reads `user` for anyone other than its owner**, and the only one that can close a customer account: `userDel` stamps `deleted` and names the admin in `deletedBy` ([ADR-048](./adr/ADR-048-an-admin-closes-a-customer-account.md)), the counterpart of the `shopOwnerDel` this same service already had for a seller. It can do nothing else to a customer — no create, no edit of their data — and the closure itself writes two fields and reads none. |
