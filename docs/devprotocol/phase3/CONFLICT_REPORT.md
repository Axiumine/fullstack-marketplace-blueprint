...
⚠️ **Overtaken by events, and the fix went the other way.** `marketplace-user/docs/nginx/` no longer exists: the edge was rewritten as three vhosts under `marketplace-nginx/` at the workspace root and the customer-only copy was deleted. Every citation this section validated — the prefixed ones included — has since been repointed at `marketplace-nginx/conf.d/`, `marketplace-nginx/snippets/` and `marketplace-nginx/sites-available/`. The audit's finding was correct when made; the file it was about is gone.
...
⚠️ **The "until" in that sentence expired on 2026-08-27, and the real ADR went the other way.** This sweep read ADR-009 as rejecting `price` *until* Cart/Order/Payment get a real ADR — a deferral. They now have one, and it refuses them: `phase3/adr/ADR-038-commerce-is-permanently-out-of-scope.md` makes cart, order, delivery and payment permanently out of scope, so there is no later ADR that unblocks `price` and ADR-009 gained a dated Note saying a display-only price was offered and refused as well. **The finding this section recorded is unchanged and is now stronger**: zero scope leakage, and no tier is coming for anything to leak toward. The (a), (b) and (c) classification of the hits still holds word for word.
