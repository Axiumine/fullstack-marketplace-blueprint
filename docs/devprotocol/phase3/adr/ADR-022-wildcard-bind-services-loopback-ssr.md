...
sits behind nginx for TLS, rate limiting (`marketplace-user/docs/nginx/rate-limit.conf`) and the
...
behind the nginx configs in `marketplace-user/docs/nginx/`, so binding wide would be a direct bypass
...
- **Risk:** nginx config for `marketplace-user` (`marketplace-user/docs/nginx/marketplace-user.conf`)
  is documentation only — `docs/architecture.md` §nginx states no nginx binary and no `/etc/nginx` exist in this
