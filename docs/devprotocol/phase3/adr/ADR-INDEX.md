...
- **Production topology.** nginx terminates TLS in front of the operator SPA per old log evidence, and `marketplace-user/docs/nginx/` carries deployable configs, but no nginx config is installed anywhere in this workspace and no ADR records the intended topology (hosts, TLS termination point, cache layer placement).
