...
- **Server-side enforcement of the gate layer.** Every gate this platform has — semgrep, trivy, the supply-chain floor, lint, type-check, 100% coverage on four metrics, 100 mutation score, Qodana — lives in `.githooks/` and runs because `core.hooksPath` and a `prepare` script wired it into a real clone ([`ADR-017`](./ADR-017-hooks-via-hookspath-and-prepare.md), [`ADR-016`](./ADR-016-hundred-percent-coverage-and-mutation.md)). ⚠️ **Nothing on the server side runs any of them**, so any merge that does not pass through a local clone passes through no gate at all: a Dependabot pull request merged from the browser is the concrete case, and [`ADR-054`](./ADR-054-the-supply-chain-score-is-a-gate-with-a-floor.md) §Risks records it as unfixable by configuration — no `dependabot.yml` field reaches it. The interim control is procedural and written at the top of all sixteen `dependabot.yml` files: fetch the branch, check it out, `yarn install`, let `pre-commit` and `pre-push` run for real, merge only then. Branch protection is applied at a deliberately low tier — force pushes and deletions blocked, `enforce_admins` false, no required reviews — because a solo maintainer with required reviews and no second maintainer cannot merge his own work; that is the accepted trade and it scores below Scorecard's maximum on purpose. **Closing this bullet means a pipeline that actually runs the gates and required status checks that depend on it**, which needs Docker-capable runners, a Qodana token in the cloud and a ruling on whether the hooks stay authoritative or become a mirror of CI. That is the platform owner's call and it is not taken here; it is the same item R39 carries as CI/CD.
