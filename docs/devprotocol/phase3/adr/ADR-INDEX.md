...
is the one lifecycle question left open, and it is recorded there rather than in an epic.
...
One row in §2, one number in §3's **Frontend** line, two rows in §4. It closes E19 §6
...
v1.16 - 2026-08-28, later still: **three rows added to §4, all from the `E15` epic record**, deleted and
...
document holds: the **lazy prune** the index's per-field `HEXPIRE` replaced, the **"revoke all but me"**
exemption a credential-change teardown does not grant, and the **`familyId`/cap fields** the index value
does not carry. Each is a platform-owner decision with a dated reason, and each is the kind a later reader
re-proposes precisely because the shipped code looks arbitrary without it.
v1.15 - 2026-08-28, later the same day: **two rows added to §4, both from the `E14` epic record**, deleted
...
missing (E19) reads clear fields only, so the temptation it creates — make a name or a
...
v1.11 - 2026-08-27: **ADR-038 added — cart, order, delivery and payment are permanently out of scope**, decided by the platform owner when asked what to do with the `E11` epic record §6 question 1. It is the ADR §5 said was owed "when the design starts", arriving because the design does not start, so the **Ordering** gap leaves §5 rather than being answered inside it. One row in §2, one number in §3's **Catalogue** line, and two rows in §4 — the existing `price` row keeps ADR-009 and gains ADR-038, because the reason changed from *no design yet* to *no design ever*. ADR-009 is **not** superseded: its decision is unchanged and its title's condition simply never arrives, which its own header now records. Corrected in the same pass: §1 said "no supersession exists", which stopped being true on 2026-08-26 when ADR-037 superseded ADR-015 in part — the sentence predates that row and nothing but the sentence was wrong. **ADR-010 also carries a dated note now** — its §Context called the cart/order model absent *"yet"*, which ADR-038 turns into absent permanently; its decision, taken on atomicity grounds that never depended on a consumer arriving, is untouched
v1.12 - 2026-08-27, later the same day: **ADR-038 gains a dated note of its own** — its §Context and §Consequences both cited `phase2/EVENT_STORMING.md` **§5** open question 4, and that question is in **§6**; §5 is the hotspot table. Both pointers now read §6, the correction is recorded at the foot of the ADR rather than made silently, and nothing it decides changed. The same wrong section number was corrected in the `E11` epic record. ⚠️ **That file no longer exists** — see v1.13 below; the correction itself is unaffected, since it lived in prose the deletion carried elsewhere rather than erased. No ADR status, decision or consequence moved.

v1.13 - 2026-08-27, later still: **The `E11` epic record is deleted; the epic id E11 is not.** ADR-038 §Consequences already said the id would survive in the epic numbering — this is that happening, on the same day it was promised. The file's record is distributed rather than replaced: ADR-038 itself gains a matching final section, §Note — 2026-08-27, *"E11's epic record deleted, its content absorbed here"*, that holds what the deleted file recorded in its own voice rather than a summary of it. §4's row for ADR-038 above cited the `E11` epic record as one of the places the four commerce concepts stay named to be "recognisable enough to refuse"; that citation is dropped rather than repointed, since the naming already stands on the three phase-2 documents the same row lists — the refusal itself is unchanged. v1.12's own citation of the file, two entries above, is annotated rather than rewritten, per this corpus's rule against silent correction. Nothing else in this index — no ADR, no §2 row, no §3 count — changed.
...
- **Where the sixteen repos get published**, and under which org. No ADR yet — it is explicitly the user's undecided call (see [`docs/workflow.md`](../../../workflow.md), *Repo layout*). ⚠️ **This is git hosting, not the npm registry.** [`ADR-037`](./ADR-037-marketplace-common-is-published-to-npm.md) decides where one *package* ships — `@axiumine/marketplace-common` to npmjs — and closes nothing here; the two were conflated once, in the `E09` epic record §6 — now [`phase5/PLATFORM_OPERATIONS_QUALITY_GATES.md`](../../phase5/PLATFORM_OPERATIONS_QUALITY_GATES.md) — which cited this bullet for a question ADR-015 §Risks had owned all along. Do not delete this bullet on the strength of ADR-037.
