...
- **Production topology.** Still owed, but narrower than it was. The edge itself is now written down: `nginx/` at the workspace root carries a vhost per hostname — apex, `shopowner.`, `admin.` — terminating TLS for all three and proxying twelve loopback processes, with `nginx/test/run.sh` exercising it in a container. What no ADR records is where that instance *runs*: which host, whether anything sits in front of it, and how the service ports are closed to everything but it (`INTROSPECTION_CODE` is reachable wherever a service port is). ⚠️ Phase 1 and Phase 3 documents cite the retired `marketplace-user/docs/nginx/*.conf` by path and line throughout; those files are deleted and the citations are history, not the current tree.
