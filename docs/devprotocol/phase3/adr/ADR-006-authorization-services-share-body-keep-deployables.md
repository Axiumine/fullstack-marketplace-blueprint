...
- **Environment-file drift across repos is structurally unenforced.** No suite spans 2 services, so nothing detects a service whose env file names another project's database, whose `INTROSPECTION_CODE` disagrees with its peers, or whose `KEYGRIP_KEY_1`/`_2` do not match `marketplace-dev-public-authorization`. Every one of those is a working service that fails only where two of them meet. Not caused by this ADR's decision; revisit only if a cross-repo contract-test mechanism is later proposed.
