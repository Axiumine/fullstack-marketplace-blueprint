...
with the standalone record's §6 question 1 — absorbed into the note at the foot of this file — who signs off the first
...
[`phase2/EVENT_STORMING.md`](../../phase2/EVENT_STORMING.md), and, until it was deleted, a standalone
record of its own — see the note at the foot of this file — that existed only to record this decision and
nothing else, because [`phase5/CONSTRAINTS.md`](../../phase5/CONSTRAINTS.md) §6 forbids any story that
designs them.
...
- **Six live open-question rows, none with a real owner.** That record's §6 questions 1–5, `BOUNDED_CONTEXT.md`
...
- **Three passes that narrowed questions rather than answering them.** That record — revised
...
[`phase5/PLATFORM_OPERATIONS_QUALITY_GATES.md`](../../phase5/PLATFORM_OPERATIONS_QUALITY_GATES.md) §6). What it demonstrates is a
...
2. **Every open question waiting on a commerce design closes as moot.** That record's §6 q1–q5,
...
3. **The commerce vocabulary stays where it is, with its framing inverted.** UL §18, ES §2.9 and BC-11
   all remain. Their job is no longer *readiness* — it is to name the four precisely enough that a reader
   or an agent recognises them and stops. This ADR carries the story now: it records the gap **and** the
...
- Six question rows collapse into one decision with a named decider. The audit loop that produced three
  narrowed-not-answered passes on that record has an exit.
...
- The four are still named in five phase-2 documents, an event-storming board and a glossary
...
A violation on disk looks like: `BC-11 [PLANNED]` anywhere, a `price` field in any builder or model, any
record with acceptance criteria that presume an order exists, a sequence diagram of a checkout flow, or
...
pass, and, while it still existed, in that record as well.
...
## Note — 2026-08-27: the standalone record deleted, its content absorbed here

This decision had a standalone record of its own, deleted on 2026-08-27. **What it held survives** —
exactly as §Consequences above already said it would — because this ADR now carries it: the remainder of
this note is what that record held. A reader who goes looking for the file and finds it gone should find
this paragraph, not silence.

**This is not the "Delete" option §Options above rejected.** That option would have erased BC-11,
`UBIQUITOUS_LANGUAGE.md` §18, `EVENT_STORMING.md` §2.9 and the standalone record itself, on the reasoning
that a reader wondering where the cart is would then find nothing explaining the wondering — which is
exactly the outcome this corpus's rule against erasing closed questions exists to prevent. None of that
happened here: the vocabulary, the reasoning and every closed question that record carried survive,
relocated rather than removed. Only the file — one address among several that once carried a live fragment
of this decision — is gone, and what it held is recorded below in this ADR's own voice rather than left to
be inferred from a title that no longer resolves.

**The absence recorded there was verified empirically, and that verification is what its §3 held.**
...
2026-08-13:** that bullet had named only eight entries; `admin.js` and `encrypted.js` were the
...
⚠️ **Correction, 2026-08-27:** that record's Delivery bullet had cited **ADR-008** since the retrofit, which was
...
**What the telemetry & egress hardening decision did not settle.** That record recorded §6 question 5 as
*narrowed, not answered*, because [`phase5/TELEMETRY_EGRESS_HARDENING.md`](../../phase5/TELEMETRY_EGRESS_HARDENING.md)
gave this platform a log-retention decision and a privacy notice, **neither of which reaches what an order
would store**. That distinction is preserved here so a future reader does not treat that retention work as
having answered the BC-11 data question — it answered a question about data this platform actually holds,
which an order is not among.
...
amends any of them. What only that record held, and what would otherwise have left with it:
...
- **Question 1's owner cell was wrong.** It read "Product + platform dev", a function this platform does
  ...
  paragraph above it.** The finding is not that record's alone any more: the twins were closed on
  2026-08-27 with the owner-cell tell written into them — `BOUNDED_CONTEXT.md` §7 q4 and
  `EVENT_STORMING.md` §6 open question 4 both carry it directly, not merely as a question's closure
  reason: an owner cell naming a function this platform does not have should be met wherever a reader
  enters.
...
This note closes on the rule that record closed on, which survives it verbatim in substance: **a future
question about cart, order, delivery or payment is not an open question on anything — it is a request to
supersede this ADR, and it goes to the platform owner as an ADR.**
