# ADR-025 — marketplace-services-status has no repo of its own and is gated by the parent workspace hooks
# Marketplace
...
`marketplace-services-status` (realtime monitor + control panel for the platform's systemd user units, see
`marketplace-services-status/package.json` `description`) is not one of the fifteen sub-repos under
...
pre-commit and in pre-push, in every sub-repo that has a `package.json`). `marketplace-services-status` has none of
that by construction. Left alone it would carry the same appearance of rigor as everyone else — `marketplace-services-status/qodana.yaml` (`testCoverageThresholds: total:
100, fresh: 100`, `failureConditions.severityThresholds: critical: 0, high: 0`),
`marketplace-services-status/stryker.config.mjs` (`thresholds: { high: 100, low: 95, break: 100 }`), a
`marketplace-services-status/vitest.config.mts` with the same 100%-on-every-metric shape as the nine backend services —
...
- The parent's ordinary commit is a docs edit — [`CLAUDE.md`](../../../../CLAUDE.md), [`README.md`](../../../../README.md), `.claude/` — and building + testing
  ...
  (`.githooks/pre-commit` comment block, "marketplace-services-status — the one piece of application code this repo
  tracks").
- `marketplace-services-status` has **no lint script** — it is not one of the thirteen eslint/prettier repos. Its type
  check is not separate either: `marketplace-services-status/package.json` `scripts.test:cov` is `"yarn build && vitest
  run --coverage"`, and `build` is `tsc`, so a type error fails the coverage gate before a single test runs
  (`docs/frontends.md` §marketplace-user block, "`marketplace-services-status` … has no `lint` script … `tsc` runs as the first
...
100 mutation score is required of every package that ships code, `marketplace-services-status` included, and the
...
`marketplace-services-status/` directly, in place of giving it a sub-repo of its own (rejected: option A), leaving it
...
`pre-commit` scopes to staged, non-markdown paths under `marketplace-services-status/` — `grep -E "^marketplace-services-status/" |
...
of habit (comment at the top of `marketplace-services-status`'s Gate 2 block in `.githooks/pre-push`).
...
`thresholds.break` in `marketplace-services-status/stryker.config.mjs`.

```
if ! yarn test:cov; then
    …
    "PUSH BLOCKED — marketplace-services-status failed to build, failed its tests, or fell below 100% coverage."
    …
```
...
### Positive
- `marketplace-services-status/qodana.yaml` and `marketplace-services-status/stryker.config.mjs` are enforced rather than
  decorative — every commit touching non-markdown `marketplace-services-status/` paths runs coverage + Qodana, every
...
- ⚠️ The gate depends on `marketplace-services-status/qodana.sh` being mode `100755`. Committed at `100644` it dies
  with `Permission denied` before reaching Qodana, and the hook then reports "Qodana failed on
  marketplace-services-status" pointing at a SARIF that was never written — a chmod bug rendered as an apparent scan
  failure. Both hooks test executability separately from file existence for that reason (ADR-017).

### Risks
- **Risk:** a future contributor gives `marketplace-services-status` a repo of its own "for consistency" and drops the
  parent-hook scoping without re-checking whether `core.hooksPath` gets wired for it. Revisit
  condition: `marketplace-services-status` gains its own `.git`, `origin`, or `package.json` `prepare` script.
...
- **Risk:** `marketplace-services-status/qodana.yaml`'s `image: jetbrains/qodana-js:2026.2` pin goes stale against a
...
change under `marketplace-services-status/`, must print `pre-commit: marketplace-services-status — building, testing and holding
100% coverage`.

Verify the app itself still holds the bar: `cd marketplace-services-status && yarn test:cov` must complete with all
four v8 metrics at 100 (thresholds live in `marketplace-services-status/vitest.config.mts`); `yarn test:mutation` must
report `break: 100` held (`marketplace-services-status/stryker.config.mjs`).

A violation looks like: a merged `marketplace-services-status/` change with `coverage/index.html` showing <100% on any
metric, or `marketplace-services-status/reports/mutation/mutation.html` showing a `Survived` mutant with no
...
violation: `marketplace-services-status/qodana.sh` committed at mode `100644` again (check with `git ls-files -s
marketplace-services-status/qodana.sh` — the mode digits must start `100755`).
