...
`item` built 2026-08-05 as the platform's domain-neutral catalogue collection (ADR-008). Builder
`BEs/marketplace-db-setup/lib/schemas/item.js`, migration
`BEs/marketplace-db-setup/migrations/20260804030000-create-item.js`. Strict `$jsonSchema`,
`additionalProperties: false`.

No cart, order, delivery or payment collection exists anywhere in
`BEs/marketplace-db-setup/migrations/` — checked, zero files match `order`/`cart` in that dir. No
...
- `Decimal128` is the correct BSON type for money and is already a **rejected write** on this
  ...
  throws — documented at `item.js:12-17` and in `BEs/marketplace-db-setup/CLAUDE.md` under *Naming*
  → geo `position` fixes (`Decimal128` was the original demo-seed coordinate type and answered 500 on
...
- `CONSTRAINTS.md` §5 states this outright as an out-of-scope boundary for the current build phase:
  "Price on `item` — deliberately absent, comment at
  `BEs/marketplace-db-setup/lib/schemas/item.js:12-17` says why. Do not add 'just a field'."
...
| C. Defer — ship `item` with no price field, design it together with `Cart`/`Order`/`Payment` when that tier is built | Matches the platform's existing "field earns its place by a reader" convention (`published`, `deleted`); avoids shipping a type known to break the `.lean()`→GraphQL path; no debt from an undesigned currency/VAT/discount decision baked into a strict-schema collection | Storefront cannot display a price today; any commerce demo waits on the ordering tier; needs a fresh alter migration (+ full DB rebuild per `lib/schemas/README.md`) whenever it does land |

---

## Decision

Option C. `item` ships with **no `price` field**, required list stays
`idCompany, idCategory, name, description, slug, published` (`item.js:48-55`), and `additionalProperties: false`
...
`lib/schemas/` edit that forces a full rebuild of every database that ran the migrations
(`BEs/marketplace-db-setup/CLAUDE.md` → *Authoring migrations*). Option C pays that migration-plus-rebuild
...
- Avoids re-introducing the `.lean()` / `GraphQLFloat.serialize(Decimal128)` failure class documented at
  `item.js:12-17`.
...
- Adding `price` later is a new `lib/schemas/item.js` shape plus an `alter-item` migration, and per
  `BEs/marketplace-db-setup/CLAUDE.md` → *Authoring migrations*, any `lib/schemas/` change is followed by a
  full rebuild of every database that ran the migrations (`dbMarketplaceDev` plus each repo's throwaway
  test DB).
...
`item.js:12-17` explanatory comment. `db.item.getSchema()` (or reading the validator programmatically)
