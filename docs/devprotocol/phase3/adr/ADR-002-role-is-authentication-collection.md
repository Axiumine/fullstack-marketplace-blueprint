...
Forcing factor: all 9 services share one Redis prefix `REDIS_KEY=marketplaceDev:` (needed structurally, see Consequences). A resource service that does `hGetAll(${REDIS_KEY}${accessToken})` and trusts any non-empty hash therefore accepts an `Admin` access token against the ShopOwner resource service, and the reverse. That is not a hypothetical risk of sharing the prefix, it is its direct consequence — which is why the session hash carries a `tier` and every resource service asserts it (ADR-004).
...
`assertTier` fails closed on a missing `tier`: `actual !== expected` has no branch for `undefined`, so a session hash carrying no `tier` at all is rejected outright rather than trusted as a wildcard — costs one re-login, not an open hole for the remaining `REFRESH_TOKEN_EXPIRY` (90 days). Mismatch answers 403, not 401: the caller authenticated correctly, just against the wrong tier, and a 401 would tell a client to refresh its way out, which it cannot.
...
- Duplication risk across the 3 authorization service bodies is real, and answered by putting the shared session/refresh/account-lookup logic in `marketplace-common` (`resolveAuthorizationSession`, `findAccountForSession`, `refreshSessionTokens`) rather than by merging the services — the deployables stay 3 on purpose (ADR-006, `docs/decisions/authorization-service-consolidation.md`).
...
### Risks
- **Missing `tier` on a session treated as trusted** — an absent `tier` that is not rejected is the whole hole this decision closes, silently reopened. Revisit trigger: any code review or diff touching `assertTier.mts` that adds an `undefined` branch, or any new service that reads the Redis hash without calling `assertTier`.
