...
`bsonType` of about forty fields.
...
either — the only `2dsphere` on the platform is `company.address.position_2dsphere`, on a public address).
...
- **`$jsonSchema` does not validate the encrypted fields.** A `binData` value cannot carry
  `maxLength: 250`, `minLength: 5` or the postcode/province length rules — the server stores a blob and
  can measure nothing about what is inside it. Those constraints hold in the application's validators
  (`src/lib/validate/`) and nowhere else, so a field that is encrypted is a field the database no longer
  backs up. `lib/schemas/encrypted.js` says so at the point where the type is chosen, which is the only
  place a reader would think to look for the missing rule.
- **The field list exists in two repos and nothing checks that they agree** — `lib/schemas/` here and
  `marketplace-common/src/encryption/encryptedFields.mts` there. Both directions of disagreement fail
  loudly at write time, which is the best that can be said for them.
