...
- **nginx is written and tested, but installed on no host.** The config lives in `marketplace-nginx/` and is exercised by `marketplace-nginx/test/run.sh` — a container that runs `nginx -t` and 168 behavioural assertions, including this bypass: MISS, then HIT, then BYPASS once a `refresh_token` cookie is present, asserting the session response is never stored. The mechanism's second half is therefore verified but undeployed (`docs/architecture.md` §nginx — there is no `/etc/nginx` on this machine). Revisit once a real install exists and the same three-step check can be run against it.
