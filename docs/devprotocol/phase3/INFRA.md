...
| 7 | Does `marketplace-common` ever get published to a real npm registry, retiring `deploy-local.sh`? | **Both halves answered `yes`, four days apart.** Publication: 2026-08-26 by [`ADR-037`](./adr/ADR-037-marketplace-common-is-published-to-npm.md) — `@axiumine/marketplace-common` is on `registry.npmjs.org`, twelve consumers resolve `^3.0.0` since 2026-08-30. Retirement: 2026-08-30 by [`ADR-047`](./adr/ADR-047-a-common-change-ships-as-a-published-release.md) — the gap between releases is closed by making a release, so `deploy-local.sh` is deleted and a change reaches a consumer as a published version or not at all. |
