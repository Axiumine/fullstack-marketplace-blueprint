...
The `marketplace-common` build paragraph now ends at `yarn build` plus the release, and no host, port,
process or unit changed.
...
- **Cross-repo agreement, unenforced by construction** — no test on this platform spans two services
  ...
  - `KEYGRIP_KEY_1`/`KEYGRIP_KEY_2` are the worked example of what closing one of these costs, and the
...
**No path around the per-request tier fence.** Every resource service's auth middleware resolves a session
or refuses the call; there is no header, code or shared value that admits a caller without one, in any tier
or environment (`NFR-SE08`, `NFR.md` §2.4). A service that needs to reach another service on this platform
holds a session like any other caller — which is why `/health` on the authenticated tiers is a
session-bearing call and a bare monitor poll gets a 412.
