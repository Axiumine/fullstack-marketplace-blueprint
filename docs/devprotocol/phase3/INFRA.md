...
| TLS termination | none — plain HTTP on every port | open — `marketplace-user/docs/nginx/marketplace-user.conf` is a documented, unbuilt candidate |
| Reverse proxy | none — frontends call backend ports directly via Vite proxy | open — same nginx configs, undeployed |
...
| 8080 | self-hosted Nominatim | infra, not a Marketplace repo | geocoding, proxied at `/geocode/` in prod nginx config | `marketplace-user/docs/nginx/cache.conf:36-42` (`upstream mkt_nominatim { server 127.0.0.1:8080; }`) |
...
log aggregator (nginx `access_log`/`error_log` write to local files per
`marketplace-user/docs/nginx/marketplace-user.conf:60-61`, with no shipping config —
`SYSTEM_CONTEXT.md` §6), and distributed tracing beyond whatever Sentry's own SDK samples. Do not describe
...
## 11. nginx — documentation only, nothing installed

Four config files exist and are deployable, at
`marketplace-user/docs/nginx/{cache.conf,marketplace-user.conf,rate-limit.conf,security-headers.conf}`.
They are the source for the customer-surface production topology candidate: TLS termination, HSTS, CSP,
the SSR upstream, a `proxy_cache` zone that bypasses on the session cookie, PMTiles range requests, and
auth-path rate-limit zones (`docs/frontends.md` §marketplace-user).

**Verify before citing further:** there is no `/etc/nginx` directory and no `nginx` binary in `PATH` on
this machine, and no nginx config is installed anywhere in this workspace.

Upstream map inside the docs-only config, all loopback — `marketplace-user/docs/nginx/cache.conf:36-42`:
```conf
...
⚠️ **This upstream map has no entry for `marketplace-admin` (3043) or `marketplace-shopowner` (3044) or
the admin-authenticated-authorization port (4025)** — expected, since these configs belong to
`marketplace-user` specifically. Whether an equivalent nginx vhost is intended for the two SPA frontends
is unconfirmed by anything on disk (`SYSTEM_CONTEXT.md` §5.11).
...
pairs with the nginx `proxy_cache` bypass-on-cookie mechanism (§11) — but since nginx is documentation-only
...
documented; no rate limiting at the network layer today — the 5 `limit_req_zone` rules
(`marketplace-user/docs/nginx/rate-limit.conf:11-26`, `NFR-SC02`) exist only in the docs-only nginx
config and are not enforced while nginx is uninstalled. `guardPublicWrite`'s Redis-backed counter
(`BEs/dev/marketplace-dev-public-resource/src/lib/access/guardPublicWrite.mts:43-50`) is the one rate
limit that **is** live in Topology A, because it runs in application code, not nginx.
...
| 3 | Is nginx actually installed in front of any of these services, using the docs-only configs in `marketplace-user/docs/nginx/`, or does another reverse proxy get chosen? | §11 — configs exist and are deployable, nothing is installed; installing them is explicitly out of scope for Phase 3 doc work (`CONSTRAINTS.md` §5). |
| 4 | Does `marketplace-admin`/`marketplace-shopowner` get an equivalent nginx vhost, or do they stay behind a different mechanism? | `SYSTEM_CONTEXT.md` §5.11 — unconfirmed by anything on disk; the only nginx docs that exist are `marketplace-user`-specific. |
...
| 10 | Is TLS terminated at nginx (per the docs-only config) or somewhere else (load balancer, CDN)? | §13 — no TLS exists in Topology A; the only TLS design on disk is inside the undeployed nginx config. |
