...
v1.8 - 2026-08-27, later still: the `E11` epic record is deleted — its knowledge distributed across the
corpus rather than left in a dedicated file, per [`ADR-038`](../phase3/adr/ADR-038-commerce-is-permanently-out-of-scope.md)
§Note. §9's ordering-related bullet was rewritten
by v1.7 below, but the sentence it replaced is quoted nowhere in this document — that verbatim wording
survived only in that record, and the record is now gone, so it is recorded here instead: the bullet
used to end **"no collection, no resolver, no schema slice, no ADR"**, before ADR-038 became that missing
ADR. This document held no reference to the `E11` record to repoint — checked and confirmed absent.
No claim changes.
