...
| `onboardingStep`/`onboardingDone` advancement | ShopOwner | ⚠️ **Corrected 2026-08-13 (E03-S04).** `shopOwnerUpdatePreferences` (Admin tier) writes both and always has — "nothing found" was a search that missed it. What no resolver does is advance either field as a side effect of the owner's own progress, and as of E03-S04 that is a decision rather than an omission | Nothing today: no frontend reads either field. When one does, an owner's step is whatever an admin last typed — `phase5/RISK_REGISTER.md` R53 |
...
| Two writers of `item.published`, no lock | Item | **nothing** — no version/lock field in schema, ⚠️ **by decision since 2026-08-14** | Last write wins, and that is the accepted answer rather than an open finding (hotspot 4, closed; `phase5/RISK_REGISTER.md` §5). ⚠️ ~~An owner's save silently restores their own value of the flag — it need not be a deliberate republish, since the owner writes the whole card and the Admin writes only the flag~~ — **struck the same day**: publishing became a separate operation on both tiers, `published` left every input object, and both writers now set that flag alone. Only a deliberate republish reverses a takedown |
...
| 6 | Should `ShopOwnerAggregate` and `UserAggregate`'s cross-BC sub-document split (BC-01 vs BC-03/BC-07, same document, no schema wall) be closed with an explicit ACL, or is "Conformist, convention only" an accepted permanent state? | Architecture | **Closed 2026-08-12 — Conformist by design, permanent, and no ACL.** One `$jsonSchema` builder and one Mongoose model per collection is a Shared Kernel: there are no two models to translate between, and `additionalProperties: false` already makes the drift an ACL absorbs impossible. What was actually unprotected was field *scope*, now enforced by E01-S10 / CON-12 — a named list plus `no-restricted-syntax` in the three ShopOwner-tier repos. `UserAggregate` gets no counterpart because `user` has no admin-only field to list |
