...
**3 auth tiers + per-tier session tier assertion** — `Admin` / `ShopOwner` / `User`, each own collection, own service pair, own Redis session `tier` field since 2026-08-05. All 9 services share one `REDIS_KEY` prefix on purpose (logout needs it), so `assertTier` is the only thing stopping a foreign-tier token — `BEs/marketplace-common/src/others/assertTier.mts:23-25` throws 403 (not 401) on mismatch, and treats a missing `tier` as invalid, never a wildcard. 3 authorization services now share their handler body via `marketplace-common@4.4.0` (`resolveAuthorizationSession`, `findAccountForSession`, `refreshSessionTokens`) while staying 3 separate deployables — decided option (c), `docs/decisions/authorization-service-consolidation.md`.
...
- **Installing nginx configs anywhere** — `marketplace-user/docs/nginx/` (`cache.conf`, `marketplace-user.conf`, `rate-limit.conf`, `security-headers.conf`) are documentation, not deployed config. No `/etc/nginx` and no nginx binary exist in this workspace or on this machine.
...
- **`marketplace-common` on an npm registry** — consumed as `@thedoctorweb_agency/marketplace-common` by package name, but 404s on `registry.npmjs.org`. Bridged locally by `BEs/marketplace-common/deploy-local.sh`, which globs this workspace and syncs `dist/` + `package.json` into every consumer's `node_modules/`. This is a standing gap, not a future-phase item with a date.
...
| An edit to `marketplace-common` is invisible to consumers until deployed | `./deploy-local.sh` re-syncs `dist/` into 9 services' `node_modules/@thedoctorweb_agency/marketplace-common/` — skipping it means the consumer keeps compiling the previous build with no error at the call site |
...
| 4 | Who installs the nginx configs in `marketplace-user/docs/nginx/` (`cache.conf`, `marketplace-user.conf`, `rate-limit.conf`, `security-headers.conf`), and on what host? No `/etc/nginx` exists in this workspace | platform owner / ops | open |
| 5 | Does `marketplace-common` ever get published to a real npm registry, retiring `deploy-local.sh`? | platform owner | open |
| 6 | Is there an admin-facing nginx vhost for `marketplace-admin`/`marketplace-shopowner`, or does 3043's TLS termination (evidenced only by a now-deleted error log) not extend to them? | platform owner | open — noted as unresolved in `docs/decisions/authorization-service-consolidation.md` §Follow-ups |
