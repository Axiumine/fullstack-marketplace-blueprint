...
**Quality-gate regime** — 100% coverage (4 metrics) + 100 mutation score in every package that ships code: 9 backend services, `marketplace-common`, `marketplace-db-setup`, 3 frontends, `services-status`. Plus `lint:check`, `tsc --noEmit`, Qodana, wired into `.githooks/pre-commit` and `.githooks/pre-push` in 15 of the 16 repos — all but `marketplace-nginx`, which has no `package.json`, no hooks and no code to gate. `core.hooksPath` is local config and must be set by hand after a fresh clone of the parent (`git config core.hooksPath .githooks`) — the 14 sub-repos self-arm via their `prepare` script, the parent has no `package.json` so nothing runs it.
