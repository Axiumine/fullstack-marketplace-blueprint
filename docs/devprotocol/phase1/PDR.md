...
- ~~**`marketplace-common` on an npm registry**~~ — **no longer a gap.** It is published to `registry.npmjs.org` at `3.0.0` and every consumer pins `^3.0.0` (`ADR-037`, 2026-08-26). ⚠️ **The `deploy-local.sh` bridge this bullet described is deleted** ([`ADR-047`](../phase3/adr/ADR-047-a-common-change-ships-as-a-published-release.md), 2026-08-30): an edit becomes visible to a consumer by being published and by nothing else, and `yarn install` — which no path may wire to anything — is authoritative in every repo. This bullet said the package 404s there; it did until 2026-08-26.
