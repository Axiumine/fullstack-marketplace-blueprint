...
retiring the script"* is reversed four days after it was written.
...
**3 auth tiers + per-tier session tier assertion** — `Admin` / `ShopOwner` / `User`, each own collection, own service pair, own Redis session `tier` field since 2026-08-05. All 9 services share one `REDIS_KEY` prefix on purpose (logout needs it), so `assertTier` is the only thing stopping a foreign-tier token — `BEs/marketplace-common/src/others/assertTier.mts:23-25` throws 403 (not 401) on mismatch, and treats a missing `tier` as invalid, never a wildcard. 3 authorization services share their handler body via `marketplace-common` (`resolveAuthorizationSession`, `findAccountForSession`, `refreshSessionTokens`) while staying 3 separate deployables — decided option (c), `docs/decisions/authorization-service-consolidation.md`.
...
| `marketplace-common` consumed by package name | Published on `registry.npmjs.org` ([`ADR-037`](../phase3/adr/ADR-037-marketplace-common-is-published-to-npm.md)), and the registry is the whole mechanism rather than a floor under a local copy ([`ADR-047`](../phase3/adr/ADR-047-a-common-change-ships-as-a-published-release.md)): an unpublished edit reaches nobody |
...
1. Clone all 16 repos, run `yarn install` in each, and have all 9 services build against `@axiumine/marketplace-common` resolved from `registry.npmjs.org` — no local copy step, on any machine.
...
|---|---|---|---|
| 1 | Where do the 16 repos get published, and under which forge org? | platform owner | open — and **asymmetric since 2026-09-06**, which is new information rather than an answer: `.gitmodules` resolves every repo to `github.com/Axiumine` by relative URL, fifteen remotes are still empty (`git ls-remote origin` returns no refs), and `marketplace-common` is **not** — its `main` and its tags from `v1.0.0` are pushed, because publishing `@axiumine/marketplace-common` needed them there. Naming a different org stays free for fifteen and is no longer free for that one |
| 2 | Do the 3 repos built 2026-08-05 (`marketplace-dev-user-authenticated-authorization`, `marketplace-dev-user-authenticated-resource`, `marketplace-user`) get a `main` branch the way the other 12 do? | platform owner | **closed 2026-09-06 — moot: they have had one from the start.** `git log --reverse main` in each names its own first commit — `90df3c1` and `c77dad2` on 2026-08-04, `b03c647` on 2026-08-05 — so `main` was never added later and never absent. All three have it checked out today and receive work on it like the other twelve. The `feat/user-tier-services` and `feat/marketplace-user` branches the question presumed exist in no repo: `git branch -a` and `git log --all` across all sixteen find neither name |
| 3 | Ordering design — cart, order, delivery, payment, and the `item.price` field they all depend on. No collection, no resolver, no schema decision exists yet | platform owner | **closed 2026-08-27 — moot: there is no ordering design and there will not be.** The four are permanently out of scope ([`ADR-038`](../phase3/adr/ADR-038-commerce-is-permanently-out-of-scope.md)) and `item` gets no `price`, transactional or display-only (ADR-009 §Note 2026-08-27). It blocked "all commerce work" and there is no commerce work to block. Same closure as `phase2/BOUNDED_CONTEXT.md` §7 q4, `phase2/EVENT_STORMING.md` §5 q4 and the five questions [ADR-038](../phase3/adr/ADR-038-commerce-is-permanently-out-of-scope.md) §Note on the deleted ordering & fulfilment record now carries |
...
| 5 | Does `marketplace-common` ever get published to a real npm registry, retiring `deploy-local.sh`? | platform owner | **closed — published, and the script is retired.** `@axiumine/marketplace-common` is on `registry.npmjs.org` and every consumer resolves it from there (`ADR-037`, `ADR-047`). `deploy-local.sh` is deleted, on the ground that a build no lockfile names is a build only one machine has |
