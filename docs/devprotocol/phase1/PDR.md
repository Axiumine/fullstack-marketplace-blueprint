...
**Quality-gate regime** — 100% coverage (4 metrics) + 100 mutation score in every package that ships code: 9 backend services, `marketplace-common`, `marketplace-db-setup`, 3 frontends, `services-status`. Plus `lint:check`, `tsc --noEmit`, Qodana, wired into `.githooks/pre-commit` and `.githooks/pre-push` in 15 of the 16 repos — all but `marketplace-nginx`, which has no `package.json` and no code to gate; it carries a `pre-push` of its own running `test/run.sh`, so all 16 gate on push and 15 also gate on commit. `core.hooksPath` is local config and must be set by hand after a fresh clone of the parent or of `marketplace-nginx` (`git config core.hooksPath .githooks`) — the 14 sub-repos that are packages self-arm via their `prepare` script, the other two have no `package.json` so nothing runs it.
