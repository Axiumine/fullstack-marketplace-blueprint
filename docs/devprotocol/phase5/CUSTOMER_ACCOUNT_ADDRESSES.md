...
`addresses[].position` had no story saying who writes it; it now has the story below, *Place an address on a
map* — the account form carries a MapLibre island with a draggable pin, so a picked geocoder suggestion places
the address and the customer can correct it or place a hand-typed one the geocoder never found. `position`
stays optional, which was the platform owner's decision when the alternative was offered: an address nothing
can place still saves.
...
is the answer, added on both sides in one piece of work, and *At most six saved addresses* is the story.
§3's "built end to end" is unchanged; this is the first thing built into this record since it was baselined.
...
v1.2 - 2026-08-25, later the same day: that finding grew into the admin active-user tables and retention
work ([`ADR-041`](../phase3/adr/ADR-041-retention-overwrites-in-place-nothing-is-destroyed.md),
[`ADR-046`](../phase3/adr/ADR-046-the-retention-window-is-an-undo-window.md),
[`ADR-049`](../phase3/adr/ADR-049-the-admin-tables-offer-the-four-account-states.md)) — which owns the
missing lever and the admin surface to reach it from, so §6's note names that work rather than leaving a
finding with nobody holding it. The decision closed above is untouched and that work does not reopen it: it
adds no gate before a customer's first login, and its customers table queries no encrypted field, so the
sentence about a moderation table reversing ADR-029 is narrowed to the *searchable* kind it was always about.
v1.5 - 2026-08-26, last that day: Four things this file does not repeat, because a reader looks for them
without knowing to check here: the address form's single `"lon,lat"` field and its three writers, the
viewport exemption that stops the map chasing a dragged pin, and the `[longitude, latitude]` order with its
six-decimal rounding are in [`docs/frontends.md`](../../frontends.md); the corrected meaning of `position` —
a hand-typed address is placed by the pin alone, rather than staying unplaced until re-picked — and
`addresses`' `maxItems: 6` are in [`phase2/UBIQUITOUS_LANGUAGE.md`](../phase2/UBIQUITOUS_LANGUAGE.md) §8,
whose two entries had described the platform as it stood before *At most six saved addresses* and *Place an
address on a map* were built. Correcting them surfaced a third stale claim, in the same glossary (§17) and
in [`phase2/EVENT_STORMING.md`](../phase2/EVENT_STORMING.md) §4: both said `me` answers "login/verify
state", and it does not — the `select` is a positive list of six fields and `GraphQLUserMe` has no field
for `emailVerify` at all, which is *Read own account via `me`*'s whole point.
...
### Fill in personal data after email confirm   `built`
...
### Update or delete an owned address only   `built`
...
### Delete an address clears a dangling default pointer atomically   `built`
...
### Set default address as one atomic pointer write   `built`
...
### Customer changes own password   `built`
...
### Every resource-service call on this tier asserts `TIER.user`   `built`
...
### Address writes validate against a strict schema   `built`
...
### At most six saved addresses   `built`
...
### Place an address on a map   `built`
...
- BC-01 (Identity & Access) ships first: `userRegister`/`verify-email-user` mint the `User` document this
  bounded context's mutations operate on — no landing-order issue for this retrofit, both already built.
...
- No `waitApprov`-equivalent exists for `User` by design (self-service) — is there any future gate
  ...
  the admin active-user tables and retention work** ([`ADR-041`](../phase3/adr/ADR-041-retention-overwrites-in-place-nothing-is-destroyed.md), [`ADR-046`](../phase3/adr/ADR-046-the-retention-window-is-an-undo-window.md), [`ADR-049`](../phase3/adr/ADR-049-the-admin-tables-offer-the-four-account-states.md))
  — all six stories built 2026-08-25, `userUpdateStatus` the lever among them. The
...
- `addresses[].position` is optional — no story here defines when/how it gets populated (client
  geocode vs manual pin). Out of this record's built scope; flagging because BC-08's map feature
  (`ShopMap.tsx`) already consumes `company.address.position`, and the customer-address analog has no
  resolver-side geocode step visible in `userAddressAdd.mts`. ⚠️ **Closed 2026-08-26 by building
  *Place an address on a map*.** The answer is *both*, in the client, and neither in the resolver: picking a geocoder
