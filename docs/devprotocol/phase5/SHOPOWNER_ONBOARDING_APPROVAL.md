...
Provision a `ShopOwner` account — by an Admin, or, since *A seller registers themselves and waits for
an admin* shipped, by the person themselves through the public site — and gate its ability to log in
behind manual approval (`waitApprov`). Own the admin-private annotation (`notes`) and the
onboarding-preference fields, none of which the ShopOwner tier itself can read or write.

⚠️ **The two provisioning routes differ in exactly one field, and it is the one this record is named
...
| `personalData.address`/`contacts` as read by any other tier | Out | still `shopOwner`-owned data, but consuming it is BC-01/BC-04's concern, not this record's |
| The activation mail and the link behind it | In | *A seller registers themselves and waits for an admin* — a self-registered account confirms its address the way a customer's does, and cannot log in until it has |
| The seller's registration form on the public site | In | *A seller registers themselves and waits for an admin* — `/register/seller` in `marketplace-user`, a page of its own beside the customer's |
...
| `company` creation on the ShopOwner's behalf | Out | BC-04 Legal Entity / Company — this record only provisions the account the FK later points at |
...
always did. Corrected 2026-08-13 by *Admin sets onboarding and session preferences*, which also closed §6's onboarding question as a deferral: the
admin's hand is the writer by decision, and the shop-owner-side flow is future work (`RISK_REGISTER.md`
R53).

## 4. Stories

### Admin provisions a ShopOwner account   `built`
...
- ⚠️ **`shopOwnerAdd` writes no `waitApprov`, and _A seller registers themselves and waits for an admin_ did not change that.** The public
...
### Admin grants or withholds login access as one full-state write   `built`
...
⚠️ **Since 2026-08-13 (the parking rule recorded in [`SESSION_TERMINATION.md`](./SESSION_TERMINATION.md) §3.1) this mutation also ends sessions, and the rule is asymmetric on purpose.**
...
free. Anyone rewriting this to compare old and new values reintroduces the window this mutation exists to close.
### Admin records an admin-private note   `built`
...
### Admin sets onboarding and session preferences   `built 2026-08-13`
...
### Admin replaces a ShopOwner's registry data   `built`
...
### Admin changes a ShopOwner's login email   `built`
...
### Admin soft-deletes a ShopOwner   `built`
...
### A seller registers themselves and waits for an admin   `built`
...
- BC-01 ([`IDENTITY_ACCESS.md`](./IDENTITY_ACCESS.md)) depends on this record: `login` cannot correctly
  refuse an unapproved or deleted account until `shopOwnerAdd`/`shopOwnerUpdateStatus`/`shopOwnerDel`
  exist and are deployed — the mutations here must land before `IDENTITY_ACCESS.md`'s approval-gate
  behaviour is testable. Both are already built, so no live ordering risk remains, but a future edit to
  either side must respect it.
- BC-04 (Legal Entity / Company) depends on this record for `company.idShopOwner` to point at a real account
  — a `company` cannot be created for a shop owner this record has not provisioned, and since 2026-08-14
...
|---|---|---|---|
| 1 | Does a freshly created `ShopOwner` start with `waitApprov` `true` (gated) or `false`/absent (ungated)? | *A seller registers themselves and waits for an admin*, 2026-08-12 | **It depends on who created it.** `shopOwnerRegister` writes it `true`, `shopOwnerAdd` writes nothing, and the schema comment that conflated "awaiting approval" with "deleted" (`EVENT_STORMING.md` §5 hotspot 1) was rewritten to say what the field actually holds and who writes it. No migration for the existing rows: every document on disk predates the public form, so every one of them was Admin-created and is correctly ungated |
| 2 | What advances `onboardingStep`/`onboardingDone` outside of an Admin manually calling `shopOwnerUpdatePreferences`? | *Admin sets onboarding and session preferences*, 2026-08-13 | **Nothing does, by decision.** `shopOwnerUpdatePreferences` (Admin tier) is the only writer and stays the only writer until a shop-owner onboarding flow is designed; the platform owner deferred that flow rather than have it invented here. The premise the question carried was also wrong in one word: it read "no mutation … writes either field", when the true statement is **no *other* mutation writes them** — a dropped "other" that had propagated into four documents and into `RISK_REGISTER.md` R27, all corrected in the same change. ⚠️ **_A seller registers themselves and waits for an admin_ raised the stakes, and that part survives the closure** as **R53** (🟢 Low). An approved self-registered account has a login and nothing else: no name, no date of birth, no address, no contacts, no company. Onboarding is the flow that would collect them and it does not exist — so today the admin either types the details in through `shopOwnerUpdate`, or the account trades under an empty `personalData`. Whoever builds it decides whether the shop-owner app writes those fields directly or asks for an Admin review, which is a product question wearing an implementation question's clothes. R53 is Low because no frontend reads either field today, so nothing is blocked while the flow is missing |
| 3 | Does self-service ShopOwner registration ever get built, or does Admin-provisioning stay permanent? | *A seller registers themselves and waits for an admin*, 2026-08-12 | **Built, and the two coexist.** Admin-provisioning did not go away — it is the route for a shop the platform recruited, and it skips the queue for that reason |
| 4 | What `idShopOwner` does an Admin-created `company` (BC-04) get absent an owning ShopOwner having created it first via this context? | 2026-08-14, outside any story | **The case the question describes cannot occur: there is no Admin-created `company` absent an owning ShopOwner.** Admin-tier `companyAdd` takes `idShopOwner: ID!` as an explicit argument, and `funCompanyAdd` (`BEs/dev/marketplace-dev-admin-authenticated-resource/src/lib/company/funCompanyAdd.mts:34-36`) resolves it before writing — `ShopOwner.exists({ _id: idShopOwner, deleted: { $exists: false } })`, 404 `shopOwner not found` otherwise. A soft-deleted owner counts as absent. So an Admin-created company is stamped with the id of a live `shopOwner` document or it is not created at all, and the order this context worried about is the enforced one: the account exists first, whether it self-registered (*A seller registers themselves and waits for an admin*) or the admin provisioned it. What survives is not a design gap but admin error — a valid id naming the *wrong* ShopOwner — and `idShopOwner` reaches the mutation from the URL of the owner detail page the admin is already on. Closed alongside `EVENT_STORMING.md` §5 hotspot 5 and §6 question 6, `DDD_AGGREGATES.md` question 3, and `RISK_REGISTER.md` **R30** |
...
| 1.4 | 2026-08-14 | The four open questions folded into one table (§6) and the changelog into one section, because every entry in them was already closed. §5's BC-04 dependency now states the enforcement v1.3 established rather than restating the old "nothing enforces the reference" |
| 1.10 | 2026-08-28, later the same day | *Admin grants or withholds login access as one full-state write* gained a note: parking a ShopOwner ends every session that account holds, releasing one revokes nothing, and the gate reads the *target state* because both flags arrive on every call, so an old-vs-new comparison would reopen the window this mutation exists to close. The surviving open question about a confirm-first email-change flow moved to [`IDENTITY_ACCESS.md`](./IDENTITY_ACCESS.md) |