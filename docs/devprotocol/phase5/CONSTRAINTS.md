...
| BCON-07 | Edit to `marketplace-common` invisible until `./deploy-local.sh` run — the registry carries releases, not edits, and a `yarn install` restores the released build over a deployed one. Story ending "committed to common" has NOT shipped. ⚠️ **This is a one-way street:** no install path in any of the 16 repos invokes `deploy-local.sh`, and none may. The collision is real only *while* an unreleased edit is deployed; otherwise a plain `yarn install` resolving `^2.0.0` is the correct result. ⚠️ **Restated 2026-08-27:** this row read *"not published to registry"*; it is published — `1.0.1` then, `2.0.0` since later that day (`ADR-037`) — which changes the reason and not the constraint. | [`docs/conventions.md`](../../conventions.md) §marketplace-common plumbing, "Consumed by package name, and published"; `BEs/marketplace-common/deploy-local.sh` | Story marks itself done at "PR merged in marketplace-common" with no deploy-local.sh step |
