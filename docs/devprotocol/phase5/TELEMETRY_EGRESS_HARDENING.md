...
> it; and `deploy-local.sh` discovers consumers *by declaration*, so three `package.json` entries are the
> whole of the plumbing. Three copies would need correcting three times, and the first correction reaching
