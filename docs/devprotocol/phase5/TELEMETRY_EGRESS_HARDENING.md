...
left: the log-sink investigation, the Sentry-capture investigation and the Cloudflare half of the Authenticated-Origin-Pulls story.
v1.3 - 2026-08-11: The log-sink investigation and the Sentry-capture investigation ran and are `built`, each with a finding under `docs/report/`.
They found eight defects the static audit could not, which become the mailed-link redaction, Redis-password, container-log-bounding, log-retention, console-log, request-body, transaction-scrubbing and environment-tag stories — one of them 🔴 and
live — and two disclosure questions that route to §6 rather than to a fix. §7 rewritten.
v1.4 - 2026-08-11: The frontend-capture story added. The transaction-scrubbing story called itself latent because no service sets a `tracesSampleRate`;
checking that against the frontends showed all three set `0.1` and configure no `beforeSend` at all, and no
frontend depends on the package the scrubber lives in. That story's note now scopes itself to the backend.
...
The log-retention story takes the period and stops waiting; the privacy-notice story is new and writes the notice, which does not exist
yet. Question 2 is settled on *whether* and open on *how*.
v1.6 - 2026-08-11: the owner chose the mechanism for question 2 — rewrite the logged path — and **the mailed-link redaction story is
...
v1.7 - 2026-08-11: **The reset-credential-fragment story added**, and the two residuals above stop being notes. The owner asked whether
...
screen.
v1.8 - 2026-08-11: The reset-credential-fragment story's **cache criterion is built**, on the owner's instruction to take that half
...
route, the hash parser and the origin's `no-store` are still to come.
...
is quoted verbatim there — the retention answer on the log-retention story with its privacy clause on the privacy-notice story, the access-log
mechanism on the mailed-link redaction story — and §6 keeps a two-row map of what was asked, what came back and where it now lives.
The edge-access-log story gains the live confirmation that used to sit in §6 as a footnote: measured, the access log carries no
client address in any field. The one thing §6 still flags is not this record's — `phase1/NFR.md` open question 1
is narrowed by these answers and not closed by them.
v2.0 - 2026-08-11: **The request-body, transaction-scrubbing and environment-tag stories are `built`**, together, because they are one edit to the
same `Sentry.init` call in the same nine files. The 🔴 is closed. Implementing the request-body story found its own first
...
spellings, and `breadcrumb.message` on every breadcrumb — and the environment-tag story's fallback is `'unknown'` rather than
`'development'`.
v2.1 - 2026-08-11: **The console-log story is `built`**. Four `console` calls gone from three services, each replaced by a
test that asserts the absence — a deleted print leaves no mutant behind it, so nothing else would notice it
coming back. What remains of the eight investigation defects is the Redis-password, container-log-bounding and log-retention stories, none of them in
a service repo.
v2.2 - 2026-08-11: **The Redis-password story and the container-log-bounding story are `built`**, together, because both defects live in
...
neither a `KEY=` shape nor a telltale extension. The log-retention story is the last of the eight.
v2.3 - 2026-08-11: **The log-retention story is `built`, and with it all eight investigation defects are fixed.**
...
behavioural — sixteen forced rotations, reading back that both removals went through `shred`. What is left is the frontend-capture story, the privacy-notice story and the rest of the reset-credential-fragment story, none of them from the investigations.
v2.4 - 2026-08-11: **The privacy-notice story is `built`** — the platform's first privacy notice, `/privacy` in
...
assertions, quotations rather than descriptions, so drift from the configuration fails the suite.
v2.5 - 2026-08-11: **The reset-credential-fragment story is `built`** — the customer's reset credential is out of the URL. The mail now
...
because that screen does not exist. Only the frontend-capture story is left.
v2.6 - 2026-08-11: **The frontend-capture story is `built`.** A frontend event was finally captured —
...
culture entropy is kept deliberately. **One correction to the reset-credential-fragment story's own gate line**: running Stryker on the
...
themselves and one equivalent mutant; fixed here, and §4 no longer claims a score nobody measured.
v2.7 - 2026-08-26: §6's closing note annotated, not rewritten. Open question 1 (NFR-CO02) was closed on 2026-08-26 by a decision taken outside this document; the 2026-08-11 paragraph saying it stayed open through this record's answers was true when written and its reasoning still holds, so it stands with a dated block quote underneath. No story, criterion or status changed. ⚠️ **Renumbered 2026-08-27.** This entry was written as `v2.6`, which another entry in this changelog already held — two different edits under one number, and a citation of "TELEMETRY_EGRESS_HARDENING.md v2.6" could not be resolved to one of them. It takes the next free number instead. It stays where it is: this changelog runs oldest-first, and the number that fits the sequence stays with the entry that sits in it. Nothing in the entry, and nothing in the document, changed with the renumber; no other document cited either number.
v2.8 - 2026-08-27: The Authenticated-Origin-Pulls story **reclassified, not closed**. Its state cell read `built` **in the repo, not yet in the zone** since 2026-08-11, which reads as outstanding work on this backlog and is not what it is: every acceptance criterion the story wrote is met and gated, and what remains — enabling Authenticated Origin Pulls in Cloudflare and placing the zone CA under `/etc/nginx/certs/` — needs a zone and a host that do not exist here. This platform is a blueprint published for the community (`ADR-037`), so that admin role does not exist in this checkout; the row now says so and names the adopter as its owner, the same reading already applied to six questions whose owner cell was the tell. ⚠️ **No score moved and no control closed.** R44 stays 3×5=15 🟠 High and R46 stays 🟠 High, because `ADR-032` forbids closing a control by appeal to a network boundary and an absent host is an absence of exposure rather than a mitigation. No story, criterion, evidence line or gate changed.
v2.10 - 2026-08-28: one dated note added, naming the topology gap that bounds the Authenticated-Origin-Pulls story. `ADR-039` supersedes `ADR-032` and decides that Cloudflare sits in front of the origin, so the configuration that story shipped is now the decided shape rather than an anticipated one. Nothing about the story moved: its repo half stays `built`, its zone half stays an adopter step (v2.8), and **R44** keeps its score and its missing date. No other section touched.
...
that is corrected — so the edge-access-log story is a prerequisite rather than a repair.
...
| Removing `sendDefaultPii` from the one service that sets it | In | closes the other half; decided in the PII-flag story |
...
| Editing or republishing `@axiumine/eslint-config-be` | Out | it needs no change — decided in the lint-guard story |
...
| Removing the `http.client_ip` span attribute the SDK writes unconditionally | In | decided in the network-derived-value story — no configuration suppresses it, so the scrubber is the only lever |
| An nginx `log_format` that omits the client address, on all three vhosts | In | decided in the edge-access-log story — a prerequisite for the `set_real_ip_from` the rate limiter needs |
| Removing `X-Real-IP` / `X-Forwarded-For` from the proxy header block | **Out** | its first entry is the only real client address in the system, and the network-derived-value story depends on knowing that |
| Dropping the `remoteip` parameter from the Turnstile verification call | In | decided in the Turnstile story — the value sent is nginx's own address |
| `set_real_ip_from` + `real_ip_header CF-Connecting-IP`, and retuning the login zones | In | decided in the real-client-address story — per-address rate limiting does not currently exist; must not land before the edge-access-log story |
| Deleting the Redis `${bucket}:ip` bucket and pinning `app.proxy` off | In | decided in the per-address-Redis story — one global bucket today, which is a live defect |
| `app.proxy = true`, or any other route by which a service learns a client address | **Out** | the decision is that only nginx learns it; the per-address-Redis story pins it with a test |
| Hashing the email before it becomes a Redis rate-limit key | In | decided in the email-hashing story — plaintext on disk in Redis, CSFLE-encrypted in Mongo, same value |
| An HMAC, or any keyed digest, for that email | Out | needs key custody, decided in [`ADR-034`](../phase3/adr/ADR-034-keygrip-keys-live-in-redis-wrapped-under-a-kek.md); the email-hashing story says plainly that SHA-256 is pseudonymisation and stops there |
| Refusing direct connections to the origin — Authenticated Origin Pulls | In | decided in the Authenticated-Origin-Pulls story — Full (strict) does not do it and neither does the certificate; mutual TLS on the Cloudflare leg does |
| A host or provider firewall on Cloudflare's ranges | Out | there is none, and the Authenticated-Origin-Pulls story replaces the need for one: mTLS needs no address list and no refresh when Cloudflare publishes new ranges |
| **Capturing** any network-derived value, in any form — truncated, hashed, salted | **Explicitly out** | the standing GDPR decision; this record only takes such values back out |
...
- **Where the headers actually land is not `event.request.headers`.** The Node HTTP integration writes them as
  ...
  the first design was refuted, and it is the single most important line in this record.
  ...
  `net.host.ip` — and `beforeSend` is not called for transactions, so none of them is scrubbed. The transaction-scrubbing story closes that.
...
- ⚠️ **The installed SDK is `@sentry/core` 10.69.0, and in it the flag no longer means "ship headers
  ...
  what the PII-flag story decides — and it decided to remove the flag.
- ⚠️ **One network-derived value is outside `dataCollection` entirely and no flag suppresses it.**
  ...
  all nine services. Only a `beforeSend` scrubber removes it — see the network-derived-value story.
- ⚠️ **`marketplace-nginx` defines no `log_format` at all**, so all three vhosts use nginx's built-in
  ...
  `admin.…conf:65`, `shopowner.…conf:64`. The edge-access-log story replaces it.
- ⚠️ **Nothing in the repo is configured for the proxy that is actually in front of it.** The zone is
  ...
  - `$remote_addr` is a **Cloudflare edge address**, so every `limit_req_zone $binary_remote_addr`
    ...
    bystander.
  - `combined` therefore logs a Cloudflare address today, not a client one. The leak the edge-access-log story prevents appears
  ...
  - The real client address does still reach the services: Cloudflare sends it, `proxy_add_x_forwarded_for`
    ...
    into `http.client_ip`. **The network-derived-value story is unaffected — it is the one place a real client address is captured
    today.**
- ⚠️ **`X-Forwarded-For` is caller-influenced end to end, and the origin lock-down does not change that.**
  ...
  telemetry is already sometimes a value a caller chose rather than an address (the network-derived-value story removes it either
  way), and **it is why the real-client-address story reads `CF-Connecting-IP` rather than `X-Forwarded-For`** — that is the one
...
- **What does refuse is `ssl_reject_handshake on` in the default server** (`conf.d/40-tls.conf:65-75`), and it
  ...
  **The real-client-address story is unaffected either way** — `set_real_ip_from` scoped to Cloudflare's ranges ignores a direct
  caller's `CF-Connecting-IP`. The Authenticated-Origin-Pulls story closes the targeted half.
- ⚠️ **`app.proxy` is off** — `new Koa()` in all nine `src/index.mts`, no assignment anywhere in the tree, and
  ...
  therefore inoperative, for two different reasons. The real-client-address story repairs the edge; the per-address-Redis story deletes the other.
- ⚠️ **The line below it does store something personal, in plaintext, on disk.** `guardPublicLogin.mts:47`
  ...
  mechanism and judged an email acceptable. The email-hashing story revisits that, and it is the one **counter** the platform
...
- `assertTurnstile` is handed the same value (`guardPublicLogin.mts:49`, `guardPublicWrite.mts:49`), so what
  ...
  the Turnstile story removes the parameter.**
- Application and access logging have **never been audited** for credential content (audit §5).

## 4. Stories

### Certificate verification is on, in all nine services   `built`
...
the wrong 204 canonises the broken logout: the tests have to be replaced, not adjusted.

### The scrubber walks where the headers actually are   `built`
...
- **`http.client_ip` is in the same removal list**, in the same three places. It is not a header attribute and
  ...
  from the `http.*.header.*` prefix misses it entirely. The network-derived-value story owns the test; this story owns the key
- The list is expressed as a **prefix-and-name policy the SDK's own filtering cannot be assumed to cover**.
  ...
  and the Sentry-major-version guard exists precisely because that behaviour can change under a bump
...
### `sendDefaultPii` is off in all nine services   `built`
...
> scrubber-placement story's scrubber still has to walk the span attributes — the SDK's own filtering is a 10.69.0
...
- The decision above is written into `docs/architecture.md`'s observability section, naming `adminUpdatePwd`
  ...
  does **not** turn on
- A test asserts the scrubber-placement story's scrubber is wired as `beforeSend` and is actually invoked, using an init-options
...
### Neither setting can come back by accident   `built`
...
- **`sendDefaultPii` is banned outright, by key**, not "when unguarded". The PII-flag story decided the flag is absent
  everywhere, so no `beforeSend` detection is needed and none is written — scrubber presence is a per-service
  test in that story, which lint cannot check across files in any case
...
### The next Sentry major will not silently reopen this   `built`
...
- The guard covers the **filtering** behaviour too, not only the flag's existence: `SENSITIVE_KEY_SNIPPETS`
  ...
  implementation detail of a minor version, and the scrubber-placement story's scrubber must never be reduced on the strength of it
...
### No network-derived value reaches telemetry   `built`
...
**domains:** backend, testing
**Acceptance criteria:**
- The scrubber-placement story's scrubber also removes the `http.client_ip` span attribute, `event.user?.ip_address`,
...
### The edge access logs record no client address   `built`
...
> **Decision, 2026-08-10, platform owner: no full IP in the access logs.** The decision behind
> the network-derived-value story extends to the edge. `marketplace-nginx` defines **no `log_format` at all** — `grep -rn 'log_format'`
...
> latter is what `@sentry/node-core` turns into `http.client_ip` (the network-derived-value story). **This story changes what is
...
> untouched. The real-client-address story is what changes the value they carry; the per-address-Redis story is what stops the services reading it.
>
> ✅ **Confirmed live, 2026-08-11.** The log-sink investigation drove the three vhosts in the test container and grepped every
...
> what the access log carried was the mailed credential (the mailed-link redaction story), and the address the error log carries comes
> from nginx's hard-coded prefix (the log-retention story), not from a format this story controls.
...
- ⚠️ **`error_log` is not templatable.** nginx builds its error entries with a hard-coded `client: <address>`
  ...
  the log-sink investigation, whose finding must record which levels emit a `client:` prefix in this configuration and what the
...
### Turnstile verification stops transmitting the client address   `built`
...
### nginx learns the real client address, and nothing downstream does   `built`
...
> of the address read here; that is the Authenticated-Origin-Pulls story's job and it does not block this.
>
...
>
> ⚠️ **The GDPR position, once the edge-access-log story, this story and the per-address-Redis story are all in place, written down rather than
> inferred, 2026-08-28.** The edge holds a binary client address in `limit_req_zone` shared memory for the
> zone's lifetime — RAM only: the edge-access-log story keeps it out of `log_format` (§4, above), the per-address-Redis story removes the
...
> available without invalidating anything above. This is the one fact the token-handling record held
> that lived nowhere else in the corpus, carried here rather than there.
...
- **The edge-access-log story is already in place when this lands**, in the same commit or an earlier one. This story is what
...
- The three login zones are retuned, because they were never enforcing what their comments claim: with the
  Redis 20/hour bucket gone (the per-address-Redis story), `mkt_auth`, `mkt_owner_auth` and `mkt_admin_auth` become the whole
...
- ⚠️ **The chosen rates are justified in the config comment against carrier and office NAT.** Nothing has
  ...
  shared address and the failure looks like a broken login, not like a rate limit. The log-sink investigation reviews it against
  real traffic
...
- `proxy_set_header X-Forwarded-For $remote_addr` replaces `$proxy_add_x_forwarded_for` in
  ...
  reads it, but `@sentry/node-core` does (the network-derived-value story), and it should not be reading a value a caller wrote
...
### The services stop counting per address   `built`
...
> **Consequence of the real-client-address story's decision.** `app.proxy` is off, so `ctx.ip` is nginx's own address and
...
> not a working one. The real-client-address story supplies the replacement.
>
...
> the email-hashing story's, which lands with this one.
...
- `grep -rn 'ctx\.ip' BEs/dev/*/src` returns zero hits — with the Turnstile story removing the Turnstile argument, this
...
### The rate-limit key carries no plaintext email   `built`
...
>
> ⚠️ **This is pseudonymisation, not anonymisation, and this document must not claim otherwise.** An email is
...
> dump is an HMAC under a managed key, and key custody is a separate decision —
> [`ADR-034`](../phase3/adr/ADR-034-keygrip-keys-live-in-redis-wrapped-under-a-kek.md); this story does not pretend to reach that.
>
> **No cutover, unlike the session-token-hashing story in [`SECURITY_AUTH.md`](../phase3/SECURITY_AUTH.md).** These keys are hour-scoped counters, not session lookups, so switching the
...
- `sha256Hex` is the single hashing primitive in `marketplace-common`, written so the session-token-hashing story's
...
### Investigation: what do the application and access logs actually contain   `built`
...
five request-scoped entries at the shipped `warn`. Yeses become the mailed-link redaction, Redis-password, container-log-bounding, log-retention
and console-log stories; the two that put a
full client address or an email address on disk routed to the platform owner rather than to a fix, and both
came back decided on 2026-08-11 — the retention answer is quoted in full on the log-retention story, the access-log
mechanism on the mailed-link redaction story.
...
- Every "yes" becomes a story below before it is called done; every "no" cites what makes it a no
...
### Investigation: capture one real Sentry event and read it   `built`
State plainly: every claim in the scrubber-placement story rests on reading `node_modules`. One captured event settles it.
...
with a plaintext password; transactions bypass `beforeSend` entirely. The scrubber-placement story's key list and fixture are
corrected in §8 there. Yeses become the request-body, transaction-scrubbing and environment-tag stories.
**domains:** backend, testing
**Acceptance criteria:**
...
- The finding confirms or corrects where the headers land, and the scrubber-placement story's key list is updated to match what was
  actually observed
- If the observed shape differs from `httpServerSpansIntegration`'s documented behaviour, the difference is
  named and the scrubber-placement story's fixture is rebuilt from the observation
**Traces:** NFR-SE08, NFR-MA07; BCON-01, BCON-04
**Evidence (gap):** audit §5 — the audit was static-only

### The observability documentation matches the code   `built`
...
- `grep -rn 'sendDefaultPii\|rejectUnauthorized' docs/` returns no statement contradicting the code after this
  hardening pass
- The log-sink investigation's finding is linked from that section, so the next audit starts from an answer rather than a blind
  spot
**Traces:** NFR-MA05, NFR-CO01; BCON-01, BCON-03
**Evidence (gap):** no observability section exists in `docs/architecture.md`

### Only Cloudflare can open a connection to the origin   `built`
...
### The mailed links stop writing a live credential to the access log   `built 2026-08-11`
...
> The access `log_format` was written to carry no address variable and it does not (the edge-access-log story, confirmed live).
...
closed, and **the reset-credential-fragment story** closes the half that is closable: the customer reset link moves into the URL
...
sink, outside this story's "access log" scope, and **now owned by the reset-credential-fragment story** together with the residual above.
...
### The Redis password stops riding in the container's argv   `built 2026-08-11`
...
### Every container log is bounded   `built 2026-08-11`
...
- ⚠️ **Rotation is not retention.** The story states plainly that bounding the file does not decide how long
  ...
  with `shred`, by the log-retention story — but it is answered for the **edge's** logs; nothing has decided a period for the
...
### The edge's log retention is pinned by the repo   `built 2026-08-11`
...
> control is lifetime rather than content. The third clause is the mailed-link redaction story's (measured, the access log holds no
> address at all — what it held was the mailed credential, now redacted), the fourth is the privacy-notice story's. This story
...
- `marketplace-nginx/CLAUDE.md` §95 and `conf.d/05-logging.conf:28-31` stop delegating the question to the log-sink investigation
...
> control**, which is also why the privacy-notice story states them in public.

### No resolver echoes its argument to a log   `built 2026-08-11`
...
- ⚠️ **This is not cosmetic while the transaction-scrubbing story is open**: console output becomes `event.breadcrumbs` on any error
  event from the same request, so a caller-controlled echo is caller-controlled telemetry. The transaction-scrubbing story landed
...
### The request body never reaches Sentry   `built 2026-08-11`
...
- All nine `src/instrument.mts` files pass `httpIntegration({ maxIncomingRequestBodySize: 'none' })`, so the
  ...
  depends on `beforeSend` running, which the transaction-scrubbing story shows is not guaranteed
- `sentryBeforeSend` deletes `event.request.data` as well, as the second layer, on the same reasoning
  the scrubber-placement story gives for not trusting the SDK's own filtering
...
- A test seeds a body containing a known password, runs a real event through the scrubber, and asserts
  `JSON.stringify(event)` does not contain it — the whole-event check the scrubber-placement story established
- The Sentry-major-version guard gains this pairing, since it is a defaults question and defaults move on a bump
...
### The scrubber runs on every event type and covers every bag that carries data   `built 2026-08-11`
...
> `beforeSendTransaction` — see the frontend-capture story, which owns the frontend half.
>
> ⚠️ **Corrected by the frontend-capture story, 2026-08-11:** the frontend half is latent too, for a different reason.
...
- `sentryBeforeSend` walks `event.breadcrumbs[].data` and `event.breadcrumbs[].message`; console output
  becomes a breadcrumb with its arguments verbatim, and the console-log story's caller-controlled echo is one.
...
- **The fixture is rebuilt from the capture**, not extended by hand: the attribute list in
  ...
  hand-written span-attribute object the scrubber-placement story shipped. The existing error-event tests stay — the capture
  confirms they model the shape that ships today
- The scrubber-placement story's text is amended where §8 of the finding corrects it: the keys live in
...
`REMOVED_KEYS` holds. It surfaced on the admin session console's key-custody audit line, where `funKeygripRotate` and `funKeygripRetire`
...
### Sentry's environment matches the deployment   `built 2026-08-11`
...
### What a frontend actually sends is measured, and scrubbed   `built 2026-08-11`
...
> The backend half of the transaction-scrubbing story is latent because no service sets a sample rate. **The frontends are not
...
**Acceptance criteria:**
- One **real captured event** per frontend kind — an error and a transaction — taken the way the Sentry-capture investigation took the
...
> what a reset link carries since the reset-credential-fragment story and a query string is what one built before it carries.
...
> the reset-credential-fragment story carries its credential there, and adding route knowledge to a shared scrubber to cover a shape that
...
### The log retention is stated where a data subject can read it   `built 2026-08-11`
**As a** customer, **when** I want to know what this platform keeps about me, **I want** the log retention
written down **so that** a decision this platform already made is also a promise made in public.

> Owner decision, 2026-08-11 — the fourth clause of the retention answer quoted in full on the log-retention story:
...
- The text matches what the log-retention story actually configures. If the two ever disagree the configuration is right and
  the page is a false statement, so the story lands **after** the log-retention story or in the same piece of work, never
  before it
...
**Evidence (owner decision):** the platform owner's retention answer of 2026-08-11, quoted in full on the log-retention story;
retention configured by the log-retention story
...
> log holds the URL and no address at all (the edge-access-log story took it out and the log-sink investigation measured that it stayed out), and
...
### The customer's reset credential leaves the URL   `built 2026-08-11`
...
> **This is the mailed-link redaction story's residual, taken seriously.** That story stopped the edge from *recording* the
...
- **Measure before changing.** `curl` the running apex at `/reset-password/<encoded-address>/<hash>` and grep
  ...
  it does not, the criterion shrinks to the key and the story says so. The Sentry-capture investigation's lesson is the reason this is
...
- **The mailed-link redaction story's two maps stay, and the story says why rather than leaving it to be re-derived**: three of the
...
- **The two `/check/verify-email*` flows.** They are Koa REST `GET`s
  ...
  consumed by the very request that writes the log line, so it is spent by the time anything stores it. The mailed-link redaction is the proportionate control there; only the reset hash has a live window (60 minutes,
...
**Traces:** NFR-SE01, NFR-SE08, NFR-CO01, NFR-CO02; BCON-01, BCON-04
**Evidence (residual):** the mailed-link redaction story above, and `docs/report/log-sink-inventory.md` §10 — the credential in the
...
- ⚠️ **The mailed-link redaction now also covers the confirm page itself**, since `^/+reset-password/.` matches
...
- **Gates.** `marketplace-dev-public-resource`: lint clean, 398 tests, 100/4, mutation 100.
  ...
  after this story, under the frontend-capture story, came back **99.86** with three survivors in `src/lib/cachePolicy.ts`:
  ...
  in the frontend-capture story's branch: the two headers are asserted once as literal bytes, and the default is now
...
- **Depends on nothing — which is why it lands first.** Every story here is independent of the other records
  in this remediation block — dependency & advisory scanning, token handling, session revocation, keygrip
  custody and the admin session console — and the certificate-verification story in particular is severe,
  near-free and landable today. Nothing in the rest of the backlog has to wait for it, and it has to wait for
  nothing.
- **Only the quality-gates record depends on it**, and loosely: its regression suite wants the scrubber in
  place before it asserts telemetry behaviour.
- **One thread runs the other way, and it is deliberate:** the email-hashing story introduces `sha256Hex` in
  `marketplace-common`, and the session-token-hashing story's `hashSessionToken` is a named wrapper over it
  rather than a second digest helper. This document lands first, so the dependency-and-advisory-scanning
  record inherits the primitive; if the order is ever reversed, the session-token-hashing story writes it and
  the email-hashing story imports it. What must not happen is two implementations, since a hashing helper is
  exactly the kind of thing that gets copied and then diverges on encoding.
- **Landing order** (BCON-05, BCON-07):
  1. All nine services — the certificate-verification story, one commit per service, no shared code needed.
  ...
  4. The lint rule (the lint-guard story), then all nine services re-linted.
  5. `marketplace-nginx` — the edge-access-log story, one commit: the `log_format`, the three `access_log`
     lines, the test container assertion. It depends on nothing above and could equally land first.
  6. `marketplace-nginx` again — the real-client-address story, `set_real_ip_from` and the retuned zones.
     **Never before step 5**; it is what makes `$remote_addr` a client address. **No longer blocked** — Full
     (strict) settles the trusted set as Cloudflare's published ranges, and the Authenticated-Origin-Pulls
     story is a different control entirely.
  7. The Turnstile, per-address-Redis and email-hashing stories together, in this order and no other:
     `marketplace-common` (drop the Turnstile parameter, drop `perIpPerHour`, add `sha256Hex` and hash the
     identity, replace the tests all three canonise), **published as a release**, then
     `marketplace-dev-public-authorization` and `marketplace-dev-public-resource`. Landing a consumer first
     breaks its build against the published signature. The three are one pass because they touch the same two
     `marketplace-common` modules and the same two guards; splitting them means three release cycles for one
     file. The per-address-Redis story should not precede step 6 either — deleting the Redis bucket before the
     edge zones work leaves a window with no per-address limit anywhere.
  8. `marketplace-nginx` a third time — the Authenticated-Origin-Pulls story. **The Cloudflare side is
  ...
  9. `docs/` — the observability-documentation story, the Sentry-major-version guard's risk row, the
     Authenticated-Origin-Pulls story's `RISK_REGISTER` row for the certificate expiry.
  10. Parent workspace — one commit bumping every touched submodule pointer (ADR-031).
  The log-sink and Sentry-capture investigations run alongside, gating nothing but their own follow-ups.

## 6. Open questions

**None open.** Every decision this record made is carried by the story that implements it, with its reasoning —
this section holds only what is still undecided.

Two questions lived here until 2026-08-11. The log-sink investigation's fourth acceptance criterion put them here rather than in
a fix, because neither was this record's to decide: they were a disclosure question and a retention question,
...
|---|---|---|
| The nginx **error** logs record a full client address at the shipped `warn`, with no retention pinned by any repo — may they, and for how long? | Kept as they are; lifetime is the control — rotate at 14–30 days, `shred` on removal; two lines of a privacy policy say so | **the log-retention story**, `built 2026-08-11` (the quote in full, why it takes 14, and the `logrotate.d/nginx` that carries it) · **the privacy-notice story**, `built 2026-08-11` (the notice at `/privacy`, quoting the same period back) |
| The nginx **access** logs record an account email address and a live one-time hash in `"$request"` — by what mechanism does that stop? | Rewrite the logged path to a redacted `$request` rather than switch the log off | **the mailed-link redaction story**, `built 2026-08-11` — at http level, because four link shapes carry the credential and only two have a `location` block |

The residual the mailed-link redaction story left — the credential still travelling in the URL, where Cloudflare and the browser
history keep it — is not an open question either. The owner asked the same day whether it could be closed
instead of accepted, and it is **the reset-credential-fragment story**, `built 2026-08-11`: for the customer reset flow the credential
...
⚠️ **One thing narrowed here was still open on 2026-08-11, and it was not this record's to close.** `phase1/NFR.md` open
...
> **Closed 2026-08-26, elsewhere.** The platform owner decided GDPR is in scope, on evidence this record never
...
> decision fifteen days later did. What this record built is unaffected, and none of it becomes a compliance
> claim: the six unimplemented obligations are `phase1/NFR.md` open question 6.

## 7. Implementation status — 2026-08-11

**All twenty-six stories are `built`, and nothing here is waiting on this workspace.** The Authenticated-Origin-Pulls
story was the last row that read as outstanding; its repo half is complete and gated, and its zone half is an adopter deployment step, reclassified 2026-08-27 (see its row below). The two investigations ran on 2026-08-11 and closed; **they
found eight defects — the mailed-link redaction, the request-body, transaction-scrubbing and environment-tag
stories, the console-log story, the Redis-password and container-log-bounding stories and the log-retention
story — and all eight are now fixed** — the mailed-link redaction first, once the
owner chose the mechanism, then the request-body, transaction-scrubbing and environment-tag stories together, because they are one edit to the same
`Sentry.init` call in the same nine files, then the console-log story, then the Redis-password and container-log-bounding stories together, because both
live in `marketplace-docker-DBs/docker-compose.yml`, and the log-retention story last, at the edge. **The 🔴 is closed**: no service with
a `DSN` set ships a request body any more. **Nothing from either investigation is open.** The last three
were not opened by an investigation either, and all three landed the day they were written: the privacy-notice story hours
after the configuration it describes, the reset-credential-fragment story once the fragment link, the static route and the parser were
built on top of the cache half, and the frontend-capture story once a browser event had actually been captured — which is what
turned its own premise, and a claim in the Sentry-capture investigation's finding, from a code-read into a measurement.
⚠️ **Fixing the Redis-password, container-log-bounding and log-retention stories corrected or extended the evidence line of each**, which is a
pattern this document keeps producing: a measurement taken to close a story is also a re-measurement of the
story — the log-retention story's version being that its central directive, `shred`, fails open on a busybox host and says
so only in cron mail. A ninth,
**the frontend-capture story**, came out of checking the transaction-scrubbing story's "latent because no sample
rate is set" claim against the frontends, where a sample rate has been set all along. A tenth,
**the privacy-notice story**, is not a defect at all: it is the second half of the owner's retention answer — the half that has to
be said in public rather than configured. An eleventh, **the reset-credential-fragment story**, comes from the owner reading the mailed-link redaction story's
residual and asking whether it can be closed rather than accepting it.

| Story | State | What is actually true |
|---|---|---|
| Certificate verification through the email-hashing story, and the observability-documentation story | `built` | Landed across the nine services, `marketplace-common` and `marketplace-nginx`, each behind that repo's own lint, 100/4 coverage, mutation and Qodana gates |
| The log-sink investigation | `built` | Measured against the running Dev stack; finding at [`docs/report/log-sink-inventory.md`](../../report/log-sink-inventory.md) v1.0. The nine application logs are clean on every planted marker, including the poisoned-credential failure paths. Four sinks are not: the access log carries the mailed one-time hash and the address, the Redis password is in the container argv, no Docker log driver is bounded, and the error log carries a full client address at `warn`. Yeses → the mailed-link redaction, Redis-password, container-log-bounding, log-retention and console-log stories; the two disclosure questions → the platform owner, both answered the same day (the log-retention story, the mailed-link redaction story) |
| The Sentry-capture investigation | `built` | One real event, built and transmitted by the real transport into a local collector; finding at [`docs/report/sentry-event-capture.md`](../../report/sentry-event-capture.md) v1.0. No header sentinel reached the wire — but the scrubber is not why, and the scrubber-placement story's three bags were empty on the event that shipped. 🔴 `event.request.data` carried the raw GraphQL body with a plaintext password; transactions bypass `beforeSend` and ship `http.client_ip` unredacted. The scrubber-placement story's key list and fixture are corrected in §8 there → the request-body, transaction-scrubbing and environment-tag stories |
| The Authenticated-Origin-Pulls story | `built` — **the remaining half is an adopter deployment step, not open work** | `snippets/origin-pull.conf` declares `ssl_client_certificate` and `ssl_verify_client on` once and is included by all four 443 server blocks, with the container suite asserting the count and that the default block is untouched. **Every acceptance criterion this story wrote is met and gated**, R44 included. What is left is not a task on this backlog: Authenticated Origin Pulls must be switched on in Cloudflare and the zone CA placed at the path the snippet names under `/etc/nginx/certs/`, **by whoever deploys this blueprint** — there is no zone here to switch it on in and no host to place a file on. This platform is a blueprint published for the community ([`ADR-037`](../phase3/adr/ADR-037-marketplace-common-is-published-to-npm.md)), so the admin role this half needs **does not exist in this checkout** — the same reading that closed six questions, where the tell was the owner cell. ⚠️ **Order matters and is not editorial:** nginx verifying first means every handshake fails from the reload, so the Cloudflare side goes on before this config is deployed — step 1 of `marketplace-nginx/README.md` §Authenticated Origin Pulls, and the trigger `RISK_REGISTER` R44 fires on. ⚠️ **Reclassifying it closes nothing and lowers no score.** R44 keeps 3×5=15 🟠 High and R46 keeps 🟠 High: [`ADR-032`](../phase3/adr/ADR-032-production-topology-owed.md) rules that no control may be argued closed by appeal to a network boundary, and "no host exists" is an absence of exposure, not a mitigation — the same trap, reached from the other side |
| The mailed-link redaction story | `built` | Landed in `marketplace-nginx` the day it was opened, once the owner picked the mechanism. Two `map` blocks in `conf.d/05-logging.conf` at http level, the request line rebuilt from parsed values, seven new assertions in the container suite, `./test/run.sh` green and mutation-checked. **Residual, stated rather than closed here and now owned by the reset-credential-fragment story:** the credential still travels in the URL, so Cloudflare's logs, the browser history and the mail client keep it. **Adjacent, same owner:** the apex caches the SSR reset page under `proxy_cache_key "$scheme$request_method$host$request_uri"`, writing the whole link to `/var/cache/nginx/marketplace-user/` for up to `inactive=24h` |
| The reset-credential-fragment story | `built` | Opened 2026-08-11, by the owner asking whether the mailed-link redaction story's residual can be closed rather than by a finding, and closed the same day across three repos. **The edge half first**: `$mkt_credential_uri` joins the session-cookie variable on `proxy_cache_bypass` / `proxy_no_cache`, so a mailed `:email/:hash` URL never becomes a cache key. Reverting it makes the second request to a reset link answer `HIT` — it was being stored, not theoretically storable. **Then the link itself**: `RESET_PATH_USER` gains a trailing `#`, so the mail points at `/reset-password/confirm#/<address>/<hash>` and the credential exists only in the browser (RFC 3986 §3.5) — no koa-utils change needed, since it normalises only `linkPath`'s leading slash and `encodeURI` never escapes `#`. `marketplace-user` reads it from `window.location.hash` through a small parser, on a static route that is `ssr: false` — the first outside `/account/*` — and the origin now answers the whole `/reset-password` prefix `private, no-store` from `src/lib/cachePolicy.ts`, moved out of the coverage-excluded `server.ts` on purpose. **Measured before and after on a production build:** the address and live hash were in the dehydration script under `public, s-maxage=60`, and are now absent from a body served `private, no-store`. **No for the two `/check/verify-email*` flows** — Koa REST `GET`s a fragment never reaches, whose hash is spent by the request that logs it. ⚠️ **Residuals, stated not closed:** links already sent keep the old shape for up to 60 minutes and now 404; the fragment stays in the browser history and the mail client forever; and `/x/reset/`, the ShopOwner reset link, **routes nowhere at all** — the panel has no reset screen, so there is no page to move a credential into |
| The Redis-password and container-log-bounding stories | `built` | Landed together on 2026-08-11 in `marketplace-docker-DBs`, the one place both defects lived. The Redis password moved out of `command:` into a generated `secrets/redis.conf` mounted read-only, and one `x-logging` anchor caps all four containers at 20 MiB × 5. Both were **re-measured after the change and again before it**, which corrected each story's own evidence: host `ps aux` never showed the password (Redis rewrites its `argv` at startup), so the before-state was two surfaces and not three; and `mdb1` had grown from 380,144 lines to 458,129 — 229 MiB in 52.6 hours — which is the rate that sized the pair. The workspace secret guard gained a `secrets/` path rule and a `requirepass` value rule, because neither existing rule would have caught a file with no `KEY=` shape and no extension |
| The log-retention story | `built` | Landed 2026-08-11 in `marketplace-nginx`, the last of the eight. `logrotate.d/nginx` — daily, `rotate 14`, `shred`, `su root adm`, `USR1` under `sharedscripts` — over a glob covering all eight destinations, nginx's own `error.log` included. Fourteen assertions, three of them behavioural: sixteen forced rotations that read back both `shred` removals out of logrotate's output, mutation-checked by dropping `shred` and halving the count. **Two things the story could not have known, both measured:** logrotate shreds through `shred … -` on the open descriptor, which busybox cannot open, so on an Alpine host the clause silently degrades to `unlink` and still exits 0 — the suite installs `coreutils` and asserts the runs are clean; and two files in `/etc/logrotate.d` matching one glob make logrotate skip the whole later-named file, so this installs **over** the packaged `nginx` rather than beside it. The error log keeps `warn` and keeps the address: lifetime is the control, and the privacy-notice story is what states it in public |
| The console-log story | `built` | Four `console` calls gone from three services on 2026-08-11: the caller-supplied argument in `publicHelloArgs`, the two login catch prints, the constant in the admin resource handler. **The absence is asserted in each place** — a deleted print leaves no mutant behind it, so nothing else would notice it returning. `loginUser.mts` was already shaped this way and is now the pattern rather than the exception |
| The request-body, transaction-scrubbing and environment-tag stories | `built` | Landed together on 2026-08-11, in `marketplace-common` and all nine services, behind each repo's lint, 100/4 coverage, mutation and Qodana gates. The 🔴 is closed: `maxIncomingRequestBodySize: 'none'` stops the body being captured and the scrubber deletes `request.data` as a second layer. **Implementing the request-body story found its own first criterion wrong** — the option is `maxIncomingRequestBodySize` on `httpIntegration`, not `maxRequestBodySize`, and the inner spelling is a silent no-op on the outer integration; the story text is corrected in place and the eslint selector matches the name that is really read. Two widenings beyond what was written, both recorded on the stories: `user-agent` and `user_agent` join the removal list, and `breadcrumb.message` is deleted on every breadcrumb rather than on `console` ones alone. **`beforeSendTransaction` is wired but unexercised in production terms** — no backend service sets a `tracesSampleRate`, so the transaction path is proved by tests and by the capture, not by a live event; the frontends were assumed to be the live case and, measured under the frontend-capture story, are not — they register no browser-tracing integration, so they ship no transaction either |
| The frontend-capture story | `built` | Opened 2026-08-11 after §6.1 of the capture, closed the same day across four repos. **Nine envelopes captured** from production builds of `marketplace-user` and `marketplace-admin` through the real `@sentry/react` transport into the §2 collector, both event kinds, a reset-password URL carrying a sentinel in the query string and another in the fragment; finding at [`docs/report/sentry-event-capture.md`](../../report/sentry-event-capture.md) §9, v1.2. `httpBodies: []` **holds**; `urlQueryParams: false` **does not** — it gates `event.request.query_string`, which the browser never fills in, while `httpContextIntegration` copies `location.href` whole onto `event.request.url`, `contexts.trace.data['url.full']`, eight span descriptions, `Referer` and both halves of the navigation breadcrumb, **which carries the reset URL onto every later event of the session**. **The story's premise was wrong and the finding says so**: `browserTracingIntegration` is not a default integration, so the apps ship no transaction at all and `0.1` is a rate applied to nothing — the transaction half is latent-until-integration, not live. `sentryBeforeSend` is published through the subpath export it already had and wired as **both** hooks in all three apps: one implementation, not three, because the frontends share no package of their own, the module imports nothing, and a consumer opts in by declaring the dependency on a published version. Every URL-valued key is truncated at the first `?` or `#`; a span `description` only when the value starts like a URL, since `lcp.element` in the same event is `p#password-hint`. Fixture built from the capture; 100/4 and mutation 100 in `marketplace-common` and all three frontends. **The frontend mutation gate had not been run since the reset-credential-fragment story and was not green**: `marketplace-user` came back 99.86 with three survivors in `src/lib/cachePolicy.ts` — two constants the suite only ever compared against themselves, and an `?? ''` default no mutant can be distinguished from — all three fixed in this branch, and the reset-credential-fragment story's gate line corrected in §4. ⚠️ **Residuals, stated not closed:** the path is not redacted, so a link built before the reset-credential-fragment story landed keeps its credential for its 60-minute window; `deviceMemory`, `hardwareConcurrency`, `effectiveConnectionType` and `contexts.culture` are kept on purpose — entropy, but not credentials and the whole payload of the product the transaction feeds; `marketplace-shopowner` was not captured, though the two that were agreed to the byte |
| The privacy-notice story | `built` | Landed 2026-08-11 in `marketplace-user`, hours after the configuration it describes. `/privacy` — SSR, indexable, one `<Link>` in the footer the root route renders on every page. The platform's **first** privacy notice: which log holds the address, which holds the URL and no address, 14 days for both, `shred` on removal, and nothing else — no lawful basis, no other retention, no access flow, because none of those has a decision behind it. Eleven assertions plus a snapshot, all of them quotations rather than `toMatch(/14 days/)`, since drifting from `logrotate.d/nginx` is the only defect this page can have. **The story's own correction:** `daily` + `rotate 14` keeps fourteen closed files beside the open one, so an entry can live fifteen days, and "14 days" alone would have been a promise the configuration overruns — the page states the mechanism and the bound |
