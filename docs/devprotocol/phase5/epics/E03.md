...
One hotspot is genuinely unresolved on disk, not an oversight of this document — see §6.
...
- What advances `onboardingStep`/`onboardingDone` outside of an Admin manually calling
  ...
  ⚠️ **E03-S08 raised the stakes on this one.** An approved self-registered account has a login and
  nothing else: no name, no date of birth, no address, no contacts, and no company. Onboarding is the
  flow that would collect them, and it does not exist — so today the operator either types the details in
  through `shopOwnerUpdate`, or the account trades under an empty `personalData`. Whoever builds it
  decides whether the shop-owner app writes those fields directly or asks for an Admin review, which is a
  product question wearing an implementation question's clothes.
