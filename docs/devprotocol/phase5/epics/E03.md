...
- Does a freshly created `ShopOwner` start with `waitApprov` `true` (gated) or `false`/absent (ungated)?
  The schema comment at `BEs/marketplace-db-setup/lib/schemas/shopOwner.js` conflates "awaiting
  approval" with "deleted" in one field's own doc comment — `EVENT_STORMING.md` §5 hotspot 1, unresolved.
  ⚠️ **This stopped being a paperwork question on 2026-08-12**, when E01-S11 made the flag refuse a login
  and every refresh: `shopOwnerAdd` sets nothing, so a new shop owner can sign in the moment they have a
  password, and the approval step only ever happens to an account an operator parks by hand. Whichever way
  it should go, it is one line in `shopOwnerAdd` plus a migration for the rows already on disk.
...
- Does self-service ShopOwner registration ever get built, or does Admin-provisioning stay permanent? Pure
  product question — [`docs/devprotocol/phase2/BOUNDED_CONTEXT.md`](../../phase2/BOUNDED_CONTEXT.md) §7 open question 1.
