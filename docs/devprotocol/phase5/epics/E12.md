...
is stored under a cache key that is the whole link, on disk, for 60s. Same credential, a different sink, and
outside this story's "access log" scope — see the residual row in §7.
...
Sixteen of twenty-five stories are `built`. The two investigations ran on 2026-08-11 and closed; **they
...
| E12-S16 | `built` | Landed in `marketplace-nginx` the day it was opened, once the owner picked the mechanism. Two `map` blocks in `conf.d/05-logging.conf` at http level, the request line rebuilt from parsed values, seven new assertions in the container suite, `./test/run.sh` green and mutation-checked. **Residual, stated rather than closed:** the credential still travels in the URL, so Cloudflare's logs, the browser history and the mail client keep it — only taking the token out of the path would remove it there, and that option was not the one chosen. **Adjacent and open:** the apex caches the SSR reset page under `proxy_cache_key "$scheme$request_method$host$request_uri"`, which writes the whole link to `/var/cache/nginx/` for 60s. Same credential, a sink this story did not cover, and no story owns it yet |
