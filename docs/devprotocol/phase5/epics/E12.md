...
### E12-S26 — The customer's reset credential leaves the URL   `partly built 2026-08-11`
...
**Built so far — the cache criterion, 2026-08-11**, on the owner's instruction to fix that half first. It is
...
**Still to build:** everything else above — the fragment link, the static route, the hash parser, `ssr: false`,
the `no-store` from the SSR origin, and the `CLAUDE.md` routing-table correction.
...
>   ✅ **Mechanism given the same day** — *"rewrite the logged path — map the two location blocks to a redacted
>   `$request` variable"* — and built: E12-S16 is `built 2026-08-11`. Question 2 is closed.
...
| 2 | ✅ **Decided and built 2026-08-11.** **The nginx access logs record account email addresses** in `"$request"`, on the customer host and the panel host (`docs/report/log-sink-inventory.md` §5). PII rather than network-derived, so the standing decision did not speak to it by its own wording. Direction given as *"anonymize access_log"*, then the mechanism: *"rewrite the logged path — map the two location blocks to a redacted `$request` variable"*. E12-S16 built it at http level rather than per location, because **four** link shapes carry `:email/:hash` and only two have a `location` block; the `Referer` carried the same pair and is redacted by a second map. Residual: the credential still travels in the URL, so Cloudflare's logs and the browser history keep it — the owner asked the same day whether that can be closed, and **E12-S26** closes the closable half by moving the customer reset link into the URL fragment | Platform owner — decided 2026-08-11 |
...
Sixteen of twenty-six stories are `built`. The two investigations ran on 2026-08-11 and closed; **they
found eight defects, E12-S16 … E12-S23, and one of the eight is now fixed** — E12-S16, the same day, once
the owner chose the mechanism. Another of them is 🔴 and live on every
...
| E12-S26 | `partly built` | Opened 2026-08-11, by the owner asking whether E12-S16's residual can be closed rather than by a finding. **The cache half is built the same day**: `$mkt_credential_uri` joins the session-cookie variable on `proxy_cache_bypass` / `proxy_no_cache`, so a mailed `:email/:hash` URL never becomes a cache key. Reverting it makes the second request to a reset link answer `HIT` — it was being stored, not theoretically storable. Six new assertions, mutation-checked in both directions. **Yes for the customer reset flow**: the link moves into the URL fragment, which no browser transmits, so the credential stops reaching the access log, the `Referer`, the cache key, Cloudflare and every middlebox in one move. **No for the two `/check/verify-email*` flows** — they are Koa REST `GET`s, a fragment never reaches them, and their hash is already spent by the time it is logged. Scoping it found two things nobody had: the SSR page **dehydrates the address and the hash into an inline `<script>`**, so the cached copy holds them in the body and not only in the key (source-level, and criterion 1 measures it); and `/x/reset/`, the ShopOwner reset link, **routes nowhere at all** — the panel has no reset screen, so the link lands on the SPA fallback's not-found |
| E12-S17 … E12-S23 | `not built` | Opened 2026-08-11 by the two investigations above. Each cites a measured observation rather than a reading, and each names the finding section it came from. **E12-S21 is the one to land first**: it is 🔴, it is one option in nine files, and until it lands any non-`GraphQLError` on a service with a `DSN` set ships the caller's request body to the collector |
