...
### E12-S12 — Investigation: what do the application and access logs actually contain   `not built`
...
### E12-S13 — Investigation: capture one real Sentry event and read it   `not built`
...
Thirteen of fifteen stories are `built`. The two that are not are investigations, and neither gates a line
of what landed.
...
| E12-S12 | `not built` | Reads what the application and access logs actually contain, against the running Dev stack. No file in this workspace produces that finding, and inventing one would be the fiction ADR-032 refuses elsewhere |
| E12-S13 | `not built` | Same shape: it captures one real Sentry event and reads it, which needs a service running against a live project. The scrubber it would check is unit-tested at 100 % mutation either way — what is missing is the observation, not the control |
