...
|---|---|---|
| The nginx **error** logs record a full client address at the shipped `warn`, with no retention pinned by any repo — may they, and for how long? | Kept as they are; lifetime is the control — rotate at 14–30 days, `shred` on removal; two lines of a privacy policy say so | **E12-S19** (the quote in full, and why it takes 14) · **E12-S25** (the notice) |
...
instead of accepted, and it is **E12-S26**.

⚠️ **One thing narrowed here is still open, and it is not this epic's to close.** `phase1/NFR.md` open
