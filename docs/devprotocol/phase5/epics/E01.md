...
- ~~No automated gate verifies `KEYGRIP_KEY_1`/`KEYGRIP_KEY_2` agreement across
  ...
  made.~~ **Decided 2026-08-12 by ADR-034; the boot half is built (E01-S12) and rotation with it
  (E01-S13). The operator screen is E01-S14, the name cleanup E01-S15.** Two
  ...
  **cannot be rotated at all** without editing five git-ignored files by hand and restarting five
  ...
  disaster path only, for a keyspace that was flushed. What remains is E01-S14, which is visibility rather
  than capability: today an operator learns a rotation landed everywhere by reading the holders hash with
  `redis-cli`.
