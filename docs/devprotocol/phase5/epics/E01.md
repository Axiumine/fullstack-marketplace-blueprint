...
- ~~No automated gate verifies `KEYGRIP_KEY_1`/`KEYGRIP_KEY_2` agreement across
  ...
  made.~~ **Decided 2026-08-12 by ADR-034; the boot half is built (E01-S12), rotation and its screen are
  E01-S13 through E01-S15.** Two
  ...
  **cannot be rotated at all** without editing five git-ignored files by hand and restarting five
  ...
  a Redis read must not become a cookie forge. ⚠️ **What is built today stops a mismatch; it does not yet
  rotate.** Five services take their keys from the record and refuse to boot without it, so the failure
  mode that reached production data can no longer reach it — but until E01-S13 lands, minting a new set
  is still `yarn seed:keygrip --force`, which logs everyone out.
