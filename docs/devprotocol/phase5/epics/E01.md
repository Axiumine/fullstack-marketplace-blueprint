...
- `KEYGRIP_KEY_1`/`KEYGRIP_KEY_2` must be identical between `marketplace-dev-public-authorization` (signs
  ...
  ⚠️ **Superseded by ADR-034, which E01-S12 builds:** the pair leaves env for one wrapped record in Redis,
  and a service that cannot unwrap it refuses to boot. Until that story lands, the manual sweep above is
  still the only check, and it covers **five** files rather than two
...
### E01-S12 — The Keygrip pair leaves five `.env` files for one wrapped record in Redis   `not built`
...
**Evidence (gap):** `new Keygrip([process.env.KEYGRIP_KEY_1!, process.env.KEYGRIP_KEY_2!], 'sha512')` at
`marketplace-dev-public-authorization/src/index.mts:135` and at `index.mts:141` in the other four

### E01-S13 — An operator rotates the signing key, and no service restarts   `not built`
...
- It unwraps, mints 64 random bytes, unshifts, retires entries older than `SESSION_CAP_DAYS_REMEMBERED`
  while never leaving fewer than two, rewraps under the bumped version in a `MULTI`, then publishes
...
**Evidence (gap):** no mutation, resolver or script on the platform writes a Keygrip key

### E01-S14 — The operator can see which service holds which key   `not built`
...
**Evidence (gap):** nothing on the platform reports which service holds which shared secret

### E01-S15 — `KEYGRIP_KEY_1`/`_2` are gone, and the documents stop describing five files   `not built`
...
**Evidence (gap):** five `env` templates still carry the pair
...
- ~~No automated gate verifies `KEYGRIP_KEY_1`/`KEYGRIP_KEY_2` agreement across
  ...
  made.~~ **Decided 2026-08-12 by ADR-034, not yet built — E01-S12 through E01-S15 own it.** Two
  ...
  **cannot be rotated at all** without editing five git-ignored files by hand and restarting five
  ...
  a Redis read must not become a cookie forge.
