...
- ~~**Nothing wraps the three write paths in a transaction, so both halves of the depth cap carry a
  ...
  **The accepted cost is contention** — two subcategories filed under one parent at the same instant now
  ...
  ⚠️ **One half stays open and cannot be closed from this service.** `funItemCategoryDelete`'s refusal over
  live *items* still races `itemAdd` on the ShopOwner tier: that write creates a document that does not
  exist yet, so the delete's transaction has nothing to collide with. Closing it means the item write paths
  in `marketplace-dev-authenticated-resource` touching the category they file under, the way this service's
  add now does — a second repo with its own gates, and not done here. Until then an item created in that
  instant can end up filed under a category retired in the same one: the items stay resolvable, which is the
  reason a category is never hard-deleted. Stated in `funItemCategoryDelete`'s own docblock so the next
  reader does not take the transaction for a guarantee it does not give.

  ADR-012 is unchanged in substance — the cap still lives in the resolver, for the reason it always did,
  since no `$jsonSchema` can read a second document. Its Risks section names only the raw-driver bypass and
  is the place this window and its answer belong.
