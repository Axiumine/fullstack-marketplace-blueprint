...
| nginx docs (`cache.conf`, `rate-limit.conf`, `security-headers.conf`, `marketplace-user.conf`) | Installing nginx anywhere on this machine | Documentation-only in this workspace — no `/etc/nginx`, no nginx binary (`docs/architecture.md` §Services) |
...
- nginx configs (documentation, not installed): `marketplace-user/docs/nginx/cache.conf`,
  `rate-limit.conf`, `security-headers.conf`, `marketplace-user.conf`.
...
- `marketplace-user/docs/nginx/cache.conf:21-30` bypasses cache on session-cookie presence via
  `map $http_cookie $mkt_user_has_session { default 0; "~*(^|;\s*)refresh_token(\.sig)?=" 1; }` — traces
  NFR-SE09. Documentation-only here (no nginx installed) — do not report this as "deployed."
...
- nginx configs under `marketplace-user/docs/nginx/` are the deploy target for whoever stands up the
  real vhost — that repo/host is outside this workspace (`docs/architecture.md` §Services).
