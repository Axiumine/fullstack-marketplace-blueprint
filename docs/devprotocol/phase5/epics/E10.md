...
**Changelog:** v1.1 - 2026-08-27: both §6 open questions closed, §6 now reads "None open".
...
- ~~`marketplace-common` has no Qodana Cloud project gap noted anywhere for itself (unlike
  ...
  accurate.** `BEs/marketplace-common` carries `qodana.yaml`, a `qodana.sh` that fails closed on a missing
  `QODANA_TOKEN` (`"QODANA_TOKEN missing or empty"`, `exit 1`) rather than degrading to an unauthenticated
  scan, and that token is present for this repo. It is **distinct from all 14 others** — each of the 15 repos
  shipping a `qodana.sh` holds a different value, compared by hash without any value being read out — and a
  Qodana Cloud token is project-scoped, so a repo-unique token is a repo-specific Cloud project. Neither hook
  carries a repo-level skip: `SKIP_QODANA` appears in `.githooks/pre-commit` and `.githooks/pre-push` only as
  the documented one-shot bypass an operator types, never as a default this repo sets. E10 was absent from the
  NFR-MA03/MA04 gap list because it does not belong in it. ⚠️ **Scope of the check:** configuration-level
  evidence — a repo-unique token and an unskipped gate — not a live scan against Qodana Cloud, which would
  mean spending the token. ⚠️ **`phase1/NFR.md` open question 4 now reads stale for a second reason:** the 4
  repos it names as project-less each hold a distinct token too, so the `SKIP_QODANA=1` workaround it
  describes looks provisioned away. Correcting that question, and `PDR.md` §8 item 8 with it, is the platform
  owner's call and is not carried by this epic.
