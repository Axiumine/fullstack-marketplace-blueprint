...
| `package.json` `exports` map (216 entries, no barrel) | Publishing to a real npm registry | `@thedoctorweb_agency/marketplace-common` 404s on `registry.npmjs.org` by design (NFR-PO04) |
...
- `BEs/dev/marketplace-dev-authenticated-authorization/src/graphQLApi/schema/mutations/refresh.mts`
  calls `refreshSessionTokens` from `@thedoctorweb_agency/marketplace-common`, not a local copy.
...
Technical story. `marketplace-common` is consumed by package name (`@thedoctorweb_agency/marketplace-common`)
...
- `BEs/marketplace-common/deploy-local.sh` globs this workspace and writes into every consumer's
  `node_modules/@thedoctorweb_agency/marketplace-common/`.
...
both `@thedoctorweb_agency/marketplace-common` and `@axiumine/koa-utils`, or a `vi.mock` of a koa-utils
...
- Every resource service under `BEs/dev/*-resource` imports its models from
  `@thedoctorweb_agency/marketplace-common/models/...`, never redeclares a schema locally.
