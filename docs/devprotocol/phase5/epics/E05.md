...
decision for `company` ([`E04.md`](E04.md) §6). The bullet now also records the asymmetry the finding never
...
- ~~Two independent writers of `item.published` (ShopOwner's `itemUpdate`, Admin's `itemUpdatePublished`)
  ...
  decided together with the same race on `company` ([`E04.md`](E04.md) §6) and recorded in
  [`../RISK_REGISTER.md`](../RISK_REGISTER.md) §5. Worth knowing when writing any story here: the owner
  ...
  owner's value of the flag**. The operator action that survives an owner's save is `itemDel` — `deleted`
  is outside `IItemUpdate`.
