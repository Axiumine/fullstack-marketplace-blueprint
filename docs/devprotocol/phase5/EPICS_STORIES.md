...
v1.61 - 2026-08-30, after that: **[`E20.md`](epics/E20.md) §6 question 6 is answered and built, and a
narrower one opens in its place.**
[ADR-051](../phase3/adr/ADR-051-a-session-exit-is-a-page-load.md): a session exit is a **page load**. Each
app builds one urql client per page load and its document cache is keyed by query and variables and by
nothing that names a session, so a sign-out that cleared the token and the session and then navigated with
the router left every cached result standing — and `shopOwnerCompanies` and `Me`, the two reads that decide
it, take no variables at all. The logout button and `onSessionLost` now end in
`window.location.assign('/')` in all three apps: six call sites, one line each, the teardown still running
first and still running whatever the server answered. A load rebuilds every module at once, which is what
makes it correct by construction rather than a list of stores to remember. ⚠️ **The exit is covered and the
entrance is not**: `marketplace-user` has no guard on `/login`, so a signed-in customer can reach the form
by ordinary in-app links and sign in as somebody else without a load — **E20's new question 7**, and the
last of this class. `E20.md` goes to v1.5, `../frontends.md` gains the rule and a recount of three suites,
and the ADR index goes to v1.27. ⚠️ **No story is added and no build state moves**: the fix is one line per
exit and changes no screen. Two E20 questions stay open — whether an admin may be **suspended**, and the
sign-in door. ⚠️ **Nothing new is merged**: three frontend branches and the parent's.
v1.60 - 2026-08-30, last that day: **[`E20.md`](epics/E20.md) §6 questions 1, 3 and 4 are answered, and
that epic is down to two open ones.** Question 1 is a **no** and now an ADR —
[ADR-050](../phase3/adr/ADR-050-the-scrub-stops-at-the-account-collections.md): the day-30 sweep runs over
`user` and `shopOwner` and no third collection, so `company.contactPerson` and `company.administrator` are
kept. They are registration data of a legal entity rather than an account's personal data, both are
`required` so `$unset` is not even available to a scrub, and either may name somebody who never held an
account here — ⚠️ **so a closed owner's name may stay legible on a company row, which is the consequence
and not a defect to be fixed by widening the sweep.** `ScrubbableTier` stays a two-member union and is the
guard. Question 3 is **built**: `unpublishOwnerStorefront` existed identically on two tiers and is now
`@axiumine/marketplace-common@3.1.0`, exported per-file, imported by both resource services at `^3.1.0`
with both local copies deleted — one rule, three call sites. ⚠️ **It is the library's first `others/`
module that imports models**, recorded in that release's changelog so the next such move is a decision
rather than a precedent. Question 4 is **accepted as it stands**: the customer undo gets no checkpoint, the
privacy notice is unchanged because nothing it says becomes false, and the exposure is written down in
`RISK_REGISTER.md` R25 rather than closed. `E20.md` goes to v1.4, `../data-model.md` gains the scrub's
stated edge, and the ADR index goes to v1.26. ⚠️ **No story is added and no build state moves**: two of the
three rulings change no behaviour at all, and are worth writing precisely because both look like something
a later reader would try to fix. Two E20 questions stay open — whether an admin may be suspended, and the
urql document cache surviving a logout. ⚠️ **Only `marketplace-common` is merged**: it is released, tagged
and published; the two service branches and the parent's are not.
v1.57 - 2026-08-30: **[`E19.md`](epics/E19.md) §6 question 3 and [`E20.md`](epics/E20.md) §6 question 5 are the same question, and both close — the Admin tier can now close a customer account.** [ADR-048](../phase3/adr/ADR-048-an-admin-closes-a-customer-account.md), on the platform owner's ruling that *"admin must act on shop owners and on customers in the same way"*: `userDel` mirrors `shopOwnerDel` argument for argument, with three divergences the data forces rather than a design chooses — no `waitApprov`, no transaction, no storefront cascade. `E19.md` goes to v1.9 and `E20.md` to v1.3; E20's §2 row now names it as built rather than as the thing missing. ⚠️ **No story is added and no build state moves in either epic**: the mutation is backend-only and has no screen, which is itself the parity — `shopOwnerDel` has had none since it was built. ⚠️ **The parity diff found a defect in E20's own work while it was at it**: `funUserDel`, the closure `/account/close` sends, read the document and then wrote it, so two closes at once could both stamp `deleted` and push the day-30 sweeper thirty days out. It is a guarded write on both tiers now. Two E19 questions stay open and four E20 ones do. ⚠️ **Nothing is merged**: two sub-repo branches and the parent's.
v1.59 - 2026-08-30, last that day: **[`E19.md`](epics/E19.md) §6 question 5 is answered and the epic has no
open question left.** [ADR-049](../phase3/adr/ADR-049-the-admin-tables-offer-the-four-account-states.md):
the table hides closed accounts on arrival and reaches them by naming the state, not by greying them in
place — and it offers **four** states rather than the two the question assumed, because `disabled` and
`deleted` are independent and an account suspended *then* closed answered to neither filter the screen had.
⚠️ **The same defect was larger on `/shopOwners`**, which had no state filter at all, so a shop owner left
that table the moment an admin suspended or closed them; both screens now carry Active, Suspended, Closed
and Closed & suspended, out of one shared table of four. Greying was refused by the index rather than by
taste: showing every state at once unbinds the leading field of `tbl_active_registeredAt` and of all four
`shopOwner.tbl_active_*`. No story moves, no collection changes and no migration was needed. **Four E20
questions stay open.**
v1.58 - 2026-08-30, last that day: **[`E19.md`](epics/E19.md) §6 question 4 is answered — yes, the privacy
notice is amended** (platform owner). The notice gains a paragraph on who inside the platform sees a
customer's data: staff read the customer base for administration, the email address included, and can
suspend or close an account. It costs no field, no index and no screen, and it is the transparency half of
what E19 built and [ADR-048](../phase3/adr/ADR-048-an-admin-closes-a-customer-account.md) completed. **One E19 question stays open** — question 5, whether the customers table
hides soft-deleted accounts by default — **and four E20 ones do.**
v1.56 - 2026-08-29, later the same day: **[`E20.md`](epics/E20.md) §6 question 2 is answered on its closure
half — an admin account cannot be closed.** The platform owner's ruling, *"admin can not be deleted !"*:
no `adminDel` on any tier, and none may be built. ⚠️ **An absence becomes a decision.** `admin` carries
`deleted` and `disabled` from `account.js`'s shared blocks and `findAccountForSession` reads both on every
refresh, so the gates were never missing — the writer was, and now nothing may become one. `admin` therefore
does not join the lifecycle at all: no closure, no thirty-day window, no undo, nothing for the day-30 sweeper
to reach. §2's E20 row and [`../data-model.md`](../../data-model.md) say so where they describe the two
closures, and `E20.md` goes to v1.2. ⚠️ **The suspension half stays open** — nothing writes `admin.disabled`
either, and an admin suspension that named its actor would need `disabledBy` and `disabledReason` migrated
onto the collection first. Five E20 questions open. ⚠️ **No ADR yet**: this is a ruling recorded in an epic
record, and a permanent refusal of ADR-038's shape would be the instrument for it.
v1.55 - 2026-08-29, later the same day: **[`E20.md`](epics/E20.md) gains a fourteenth story, E20-S14 — the
customer's own close-account screen** — and §2's E20 row is rewritten around it. `userDel` was built on
2026-08-26 and **no application on the platform called it**, so the customer half of the account lifecycle
was a resolver, a day-30 sweeper and a privacy page describing an act nobody could perform;
`marketplace-user` now carries `/account/close`, a fourth screen in the private area, sending the mutation
with an empty variable set and signing the customer out. ⚠️ **A story, not a decision** — every consequence
the screen states is ADR-041, ADR-044 or ADR-046 already, and it states them because a refused login is
deliberately generic on every tier, which buys silence at the login screen and has to be paid for in words
somewhere. ⚠️ **And E20's build state stops saying nothing is merged**: the twelve branches of its v1.0 were
merged into `main` in their own repositories on the owner's word, with the parent bumping the twelve pointers
in `d403e3e`. `E20.md` goes to v1.1 and `E19.md` to v1.8, the latter because §6 question 3's customer half —
"a customer can close their own account" — was true of the API and false of anything a customer could reach
until today; what stays open there is the Admin counterpart alone. Six E20 questions stay open, none of them
this one's.
v1.54 - 2026-08-29, later the same day: **[`E19.md`](epics/E19.md) §6 question 2 is answered and built, and it is the first of that epic's five to close by being built rather than by being corrected.** The platform owner asked for the customer counterparts of the shop-owner pair, and `usersStats` and `usersPerPeriod` now exist on the Admin resource service, served by a new `registeredAt_series` index on `user`, with a counter and a registrations chart above the `/customers` table. Three sub-repos and the parent — `marketplace-db-setup`, `marketplace-dev-admin-authenticated-resource`, `marketplace-admin` — exactly E19's own set. §2's E19 row gains the outcome and `E19.md` goes to v1.7; `phase4/ERD.md` goes to v1.7 for the index, and its "no `registeredAt_series` counterpart" sentence is struck rather than deleted. ⚠️ **This closes a question, it does not add a story**: E19 still has six, five built and the sixth an anti-story, and E19-S05 is untouched — the screen that can now chart the customer base still refuses a search box, because ADR-029 blocks *matching and ordering* ciphertext and neither counting nor bucketing `registeredAt` is either. Four E19 questions stay open. ⚠️ **Nothing is merged**: three branches, none pushed.
...
admin a switch, and what the switch *means* was open behind it. Of the two E19 §6 questions it reaches,
...
consumer). §2.1's E17 row absorbs the five-step landing order and the constraint underneath it: **an admin
...
v1.51 - 2026-08-28, later still: **E17 question 5 is answered — the admin is named by digest.** The owner
...
`SESSION_CAP_DAYS_REMEMBERED = 30` — and found that E17 decides admin attribution *twice*: question 4 rules
it out for `funRevokeSession`/`funRevokeAllSessions`, which write no admin id anywhere, while E17-S08's
...
code. A platform admin can now list the sessions an account holds, end one or all of them, and read the
...
of S05: the admin editing somebody else's address keeps their own session, because the caller rule is
about whose credentials changed. The story's investigation found **the confirm-an-email-change flow has
no caller** — `emailVerify.newEmailTmp` is in the schema and written by nothing — so an admin's write
...
an audit.** [`E19.md`](epics/E19.md) — Customer Administration — is the admin's missing surface over the
...
| §3.6b no account→sessions index | E15, surfaced by E17; closed 2026-08-13 by E15-S02 (written), E15-S03 (pruned) and E17-S02 (read by an admin) |
