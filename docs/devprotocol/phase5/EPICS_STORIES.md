...
half — an operator account cannot be closed.** The platform owner's ruling, *"admin can not be deleted !"*:
...
either, and an operator suspension that named its actor would need `disabledBy` and `disabledReason` migrated
