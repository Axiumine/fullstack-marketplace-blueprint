...
This project is indexed by GitNexus as **`fullstack-marketplace-blueprint`** — this parent directory only, holding the workspace docs and no application code. ⚠️ Since ADR-031 the fifteen sub-repos are submodules, so their paths had to leave `.gitignore` and what keeps them out of this index is **`.gitnexusignore`** at the workspace root — a new sub-repo needs a line there too, or the next `analyze` swallows the whole platform into this one index (it did once; see `docs/gitnexus.md` §Two registry names that will mislead you). The platform's code lives in **fourteen** separate sub-repo indexes — `marketplace-nginx`, the fifteenth sub-repo, ships nginx config rather than code and is not indexed — tied together by the group `marketplace-platform`; see **`docs/gitnexus.md`** for the registry-name table, the `repo:` parameter rules and the contract-extraction gap. Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
